{
  description = "BharatCode - an AI agent CLI";
  ...
  outputs = { self, nixpkgs, flake-utils, rust-overlay }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        cargoToml = builtins.fromTOML (builtins.readFile ./crates/bharatcode-cli/Cargo.toml);
      ...
      in
      {
        packages.default = pkgs.rustPlatform.buildRustPackage {
          ...
          cargoBuildFlags = [ "--package" "bharatcode-cli" ];
          ...
          checkPhase = ''
            ...
            cargo test --package bharatcode-cli --release
          '';
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            echo "BharatCode development environment"
            echo "Rust version: $(rustc --version)"
            echo ""
            echo "Commands:"
            echo "  nix build                 - Build BharatCode CLI"
            echo "  nix run                   - Run BharatCode CLI"
            echo "  cargo build -p bharatcode-cli - Build with cargo"
            echo "  cargo run -p bharatcode-cli   - Run with cargo"
          '';
        };
      }
    );
