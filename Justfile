...
[unix]
release-windows:
    @echo "just release-windows requires a Windows host because BharatCode Windows releases build the MSVC target. Use .github/workflows/bundle-desktop-windows.yml for CI builds."
    @exit 1

[windows]
release-windows:
    @powershell.exe -NoProfile -ExecutionPolicy Bypass -Command 'rustup target add x86_64-pc-windows-msvc; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; cargo build --release --target x86_64-pc-windows-msvc -p goose-server; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; Write-Host "Windows executable created at ./target/x86_64-pc-windows-msvc/release/bharatcoded.exe"'
...
copy-binary BUILD_MODE="release":
    @if [ -f ./target/{{BUILD_MODE}}/bharatcoded ]; then \
        echo "Copying bharatcoded binary from target/{{BUILD_MODE}}..."; \
        cp -p ./target/{{BUILD_MODE}}/bharatcoded ./ui/desktop/src/bin/; \
    else \
        echo "Binary not found in target/{{BUILD_MODE}}"; \
        exit 1; \
    fi
    @if [ -f ./target/{{BUILD_MODE}}/bharatcode ]; then \
        echo "Copying bharatcode CLI binary from target/{{BUILD_MODE}}..."; \
        cp -p ./target/{{BUILD_MODE}}/bharatcode ./ui/desktop/src/bin/; \
    else \
        echo "bharatcode CLI binary not found in target/{{BUILD_MODE}}"; \
        exit 1; \
    fi

# Copy binary command for Intel build
copy-binary-intel:
    @if [ -f ./target/x86_64-apple-darwin/release/bharatcoded ]; then \
        echo "Copying Intel bharatcoded binary to ui/desktop/src/bin with permissions preserved..."; \
        cp -p ./target/x86_64-apple-darwin/release/bharatcoded ./ui/desktop/src/bin/; \
    else \
        echo "Intel release binary not found."; \
        exit 1; \
    fi
    @if [ -f ./target/x86_64-apple-darwin/release/bharatcode ]; then \
        echo "Copying Intel bharatcode CLI binary to ui/desktop/src/bin..."; \
        cp -p ./target/x86_64-apple-darwin/release/bharatcode ./ui/desktop/src/bin/; \
    else \
        echo "Intel bharatcode CLI binary not found."; \
        exit 1; \
    fi
...
[windows]
copy-binary-windows:
    @powershell.exe -NoProfile -ExecutionPolicy Bypass -Command 'if (Test-Path ./target/x86_64-pc-windows-msvc/release/bharatcoded.exe) { \
        Write-Host "Copying Windows binary to ui/desktop/src/bin..."; \
        New-Item -ItemType Directory -Force "./ui/desktop/src/bin" | Out-Null; \
        Copy-Item -Path "./target/x86_64-pc-windows-msvc/release/bharatcoded.exe" -Destination "./ui/desktop/src/bin/" -Force; \
    } else { \
...
run-ui-playwright:
    ...
    RUN_DIR="$HOME/bharatcode-runs/$(date +%Y%m%d-%H%M%S)"
    mkdir -p "$RUN_DIR"
...
debug-ui:
    @echo "🚀 Starting BharatCode frontend in external backend mode"
    cd ui/desktop && \
...
debug-ui-main-process:
	@echo "🔍 Starting BharatCode UI with main process debugging enabled"
	@just release-binary
	cd ui/desktop && \
	pnpm install && \
...
run-server:
    @echo "Running server..."
    cargo run -p goose-server --bin bharatcoded agent
...
[unix]
make-ui-windows:
    @echo "just make-ui-windows requires a Windows host because BharatCode Windows releases build the MSVC target. Use .github/workflows/bundle-desktop-windows.yml for CI builds."
    @exit 1
...
record-mcp-tests: build-test-tools
  BHARATCODE_RECORD_MCP=1 cargo test --package goose --test mcp_integration_test
