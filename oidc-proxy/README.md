...
| `OIDC_AUDIENCE` | The audience your workflow requests (e.g. `bharatcode-oidc-proxy`) |
...
steps:
  - name: Get OIDC token
    ...
    with:
      script: |
        const token = await core.getIDToken('bharatcode-oidc-proxy');
  ...
  - name: Call API through proxy
    ...
    run: bharatcode run --recipe my-recipe.yaml
```
