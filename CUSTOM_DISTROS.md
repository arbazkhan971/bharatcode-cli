# Custom Distributions of BharatCode
...
This guide explains how to create custom distributions of BharatCode tailored to your organization's needs—whether that's preconfigured models, custom tools, branded interfaces, or entirely new user experiences.

## Overview

BharatCode's architecture is designed for extensibility. Organizations can create "remixed" versions that:
...
```bash
git clone https://github.com/YOUR_ORG/bharatcode.git
cd bharatcode
```
...
BharatCode is licensed under Apache License 2.0 (ASL v2). It is a derivative work
of the upstream Goose project (Copyright Block, Inc.); see [`NOTICE`](NOTICE) and
[`MODIFICATIONS.md`](MODIFICATIONS.md). Custom distributions must:
- Include the original license and copyright notices
- Clearly indicate any modifications made
- Note that the Apache-2.0 license does **not** grant rights to any third-party
  trademarks. Upstream names and logos (such as "Goose" and "Block") remain the
  property of their respective owners, and "BharatCode" should not be used in ways
  that imply official endorsement.
...
BharatCode includes optional telemetry (via PostHog) to help improve the project. For custom distributions, you can:
- **Disable telemetry**: Set `BHARATCODE_DISABLE_TELEMETRY=1`
...
**Goal**: Ship BharatCode preconfigured to use a local Ollama model, requiring no API keys.
...
BHARATCODE_PROVIDER: ollama
BHARATCODE_MODEL: qwen3-coder:latest
```

2. **Set environment defaults** in your launcher script or packaging:

```bash
export BHARATCODE_PROVIDER=ollama
export BHARATCODE_MODEL=qwen3-coder:latest
...
**Goal**: Distribute BharatCode internally with pre-provisioned API keys for a frontier model.

### Steps

1. **Store API keys securely** using BharatCode's secret management:

```yaml
# config.yaml (distributed with your package)
BHARATCODE_PROVIDER: anthropic
BHARATCODE_MODEL: claude-sonnet-4-20250514
```

2. **Inject secrets at install time** or via your MDM/configuration management:

```bash
# Secrets are stored in system keyring or ~/.config/bharatcode/secrets.yaml
# if BHARATCODE_DISABLE_KEYRING=1
bharatcode configure set-secret ANTHROPIC_API_KEY "your-corporate-key"
```
...
- Config file location: `~/.config/bharatcode/config.yaml`
...
description: BharatCode configured for data analysis
instructions: |
...
extensions:
  - type: stdio
    name: internal-data
    cmd: python
    args: ["/opt/corp-bharatcode/internal_data_mcp.py"]
...
export GITHUB_REPO="your-bharatcode-fork"
export GOOSE_BUNDLE_NAME="InsightStream-bharatcode"
```
...
**Goal**: Create an entirely new frontend while leveraging BharatCode's backend.

BharatCode provides two integration options for building custom UIs:
...
**Handle streaming responses** - BharatCode uses Server-Sent Events (SSE) for real-time responses.
...
**Start BharatCode as an ACP agent**:

```bash
# Run BharatCode as an ACP server on stdio
bharatcode acp --with-builtin developer,memory
...
class AcpClient:
    def __init__(self):
        self.process = subprocess.Popen(
            ['bharatcode', 'acp', '--with-builtin', 'developer'],
...
**Goal**: Create a version of BharatCode tailored for a specific professional audience.
...
extensions:
  ...
  - type: stdio
    name: legal-database
    cmd: python
    args: ["/opt/legal-bharatcode/legal_db_mcp.py"]
...
Create a JSON file in `~/.config/bharatcode/custom_providers/` or bundle in your distribution:
...
Recipes are YAML files that define complete BharatCode experiences—instructions, extensions, parameters, and prompts bundled together. They're ideal for custom distributions because they require no code changes and can be distributed as simple files.
...
For complex workflows, BharatCode supports two powerful composition mechanisms:
