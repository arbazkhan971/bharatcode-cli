...
alias @bharatcode='{goose_bin} term run'
alias @g='{goose_bin} term run'

bharatcode_preexec() {
    [[ "$1" =~ ^bharatcode\ term ]] && return
    [[ "$1" =~ ^(@bharatcode|@g)($|[[:space:]]) ]] && return
    ('{goose_bin}' term log "$1" &) 2>/dev/null
}

if [[ -z "$bharatcode_preexec_installed" ]]; then
    bharatcode_preexec_installed=1
    trap 'bharatcode_preexec "$BASH_COMMAND"' DEBUG
fi{command_not_found_handler}"#,
    command_not_found: Some(
        r#"

command_not_found_handle() {
    echo "🪿 Command '$1' not found. Asking bharatcode..."
    '{goose_bin}' term run "$@"
    return 0
}"#,
    ),
...
add-zsh-hook preexec bharatcode_preexec{command_not_found_handler}"#,
    command_not_found: Some(
        r#"
...
function @bharatcode; {goose_bin} term run $argv; end
function @g; {goose_bin} term run $argv; end

function bharatcode_preexec --on-event fish_preexec
    string match -q -r '^bharatcode term' -- $argv[1]; and return
    string match -q -r '^(@bharatcode|@g)($|\s)' -- $argv[1]; and return
...
def --wrapped @bharatcode [...args] { run-external "{goose_bin}" "term" "run" ...$args }
def --wrapped @g [...args] { run-external "{goose_bin}" "term" "run" ...$args }

if (($env | get -o BHARATCODE_NU_PREEXEC_INSTALLED | default false) != true) {
    $env.BHARATCODE_NU_PREEXEC_INSTALLED = true
    $env.config.hooks.pre_execution = (
        ...
        | append {||
            ...
            if ($line =~ '^bharatcode term(\s|$)') {
                return
            }
            if ($line =~ '^(@bharatcode|@g)(\s|$)') {
                return
            }
...
$env.config.hooks.command_not_found = {|command_name|
    ...
    print $"🪿 Command '($command_name)' not found. Asking bharatcode..."
...
static POWERSHELL_CONFIG: ShellConfig = ShellConfig {
    script_template: r#"$env:AGENT_SESSION_ID = "{session_id}"
function @bharatcode {{ & '{goose_bin}' term run @args }}
function @g {{ & '{goose_bin}' term run @args }}

Set-PSReadLineKeyHandler -Chord Enter -ScriptBlock {{
    $line = $null
    [Microsoft.PowerShell.PSConsoleReadLine]::GetBufferState([ref]$line, [ref]$null)
    if ($line -notmatch '^bharatcode term' -and $line -notmatch '^(@bharatcode|@g)($|\s)') {{
...
pub async fn handle_term_init(
    ...
    let session = match named_session {
        ...
        None => {
            let session = session_manager
                .create_session(
                    working_dir,
                    "BharatCode Term Session".to_string(),
                    SessionType::Terminal,
                    Config::global().get_bharatcode_mode().unwrap_or_default(),
                )
                .await?;
    ...
    let goose_bin = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "bharatcode".to_string());
...
pub async fn handle_term_log(command: String) -> Result<()> {
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set. Initialize terminal integration with `bharatcode term init <shell>` and reload your shell first."
        )
    })?;
    ...
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set.\n\n\
             Initialize terminal integration with `bharatcode term init <shell>` in your shell profile, \
...
/// Handle `bharatcode term info` - print compact session info for prompt integration
pub async fn handle_term_info() -> Result<()> {
    ...
    let model_name = config
        ...
        .map(|name| {
            ...
            if let Some(stripped) = short.strip_prefix("bharatcode-") {
                stripped.to_string()
            } else {
                short.to_string()
            }
...
mod tests {
    ...
    fn render_term_init_script_includes_nushell_hooks() {
        let script = render_term_init_script(Shell::Nu, "session-123", "/tmp/bharatcode", false);

        assert!(script.contains("$env.AGENT_SESSION_ID = \"session-123\""));
        assert!(script.contains("def --wrapped @bharatcode [...args]"));
        assert!(script.contains("def --wrapped @g [...args]"));
        assert!(script.contains("BHARATCODE_NU_PREEXEC_INSTALLED"));
        assert!(script.contains("$env.config.hooks.pre_execution"));
        assert!(script.contains("job spawn { run-external \"/tmp/bharatcode\" \"term\" \"log\" $line | complete | ignore } | ignore"));
        assert!(!script.contains("command_not_found = {|command_name|"));
    }

    #[test]
    fn render_term_init_script_includes_nushell_default_handler() {
        let script = render_term_init_script(Shell::Nu, "session-123", "/tmp/bharatcode", true);

        assert!(script.contains("$env.config.hooks.command_not_found = {|command_name|"));
        assert!(script.contains(
            "run-external \"/tmp/bharatcode\" \"term\" \"run\" $prompt | complete | ignore"
        ));
    }

    #[test]
    fn render_term_init_script_skips_unsupported_default_handler() {
        let script = render_term_init_script(Shell::Fish, "session-123", "/tmp/bharatcode", true);
