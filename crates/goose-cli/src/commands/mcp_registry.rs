...
//!                     validated by `goose::plugins::mcp_servers`).
//!
...
/// `mcpServers` document shape that `goose::plugins::mcp_servers` validates.
///
...
mod tests {
    ...
    fn snippet_round_trips_to_valid_document() {
        for entry in all() {
            let snippet = to_extension_snippet(entry);
            goose::plugins::mcp_servers::validate_mcp_server_document(&snippet).unwrap_or_else(
