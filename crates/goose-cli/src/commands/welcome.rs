...
//!   * provider/model — [`goose::config::providers::get_active_provider`] /
//!                      [`goose::config::providers::get_active_model`]
...
fn steps() -> Vec<Step> {
    ...
    let model = goose::config::providers::get_active_model(config)
...
mod tests {
    ...
    fn dry_run_writes_nothing_to_a_temp_config() {
        ...
        let cfg = goose::config::Config::new_with_file_secrets(&config_path, &secrets_path)
            .expect("temp config");
