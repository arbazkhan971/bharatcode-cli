...
//! [`goose::skills`]) and the canonical page manifest
//! ([`goose::doc_manifest`]), which is the single source of truth that seeds the
...
/// [`goose::skills::discover_skills`], filtered to the built-in (embedded)
...
fn render_skills_page() -> String {
    let mut entries: Vec<(String, String)> = goose::skills::discover_skills(None)
        .into_iter()
