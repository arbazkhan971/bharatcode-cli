...
pub fn get_newline_key() -> char {
    Config::global()
        .get_param::<String>("BHARATCODE_CLI_NEWLINE_KEY")
        .ok()
...
///
/// When `bharatcode_prompt_editor` is configured, defaults to `true` (backward compat).
/// Users can override by explicitly setting `bharatcode_prompt_editor_always` to `false`.
...
pub fn get_input(
    ...
    let prompt_editor = config.get_bharatcode_prompt_editor().ok().flatten();
    let editor_always_override = config.get_bharatcode_prompt_editor_always().ok().flatten();
    let editor_always = should_use_editor_always(prompt_editor.as_deref(), editor_always_override);

    if editor_always {
        if let Ok(Some(editor_cmd)) = config.get_bharatcode_prompt_editor() {
    ...
    // Apply the user-customizable interactive bindings (BHARATCODE_KEYS).
    // Newline is always (re)bound — its default (ctrl-j) reproduces the prior
    // built-in behavior exactly while letting an override take effect.
    if let Some(newline) = keybindings.newline_key() {
        editor.bind_sequence(
            newline,
            rustyline::EventHandler::Simple(rustyline::Cmd::Newline),
        );
    }
    // Submit / history are only rebound when the user overrides them, so the
    // default editor behavior stays byte-for-byte identical otherwise.
    if let Some(submit) = keybindings.submit_key() {
        if submit != rustyline::KeyEvent(rustyline::KeyCode::Enter, rustyline::Modifiers::NONE) {
            editor.bind_sequence(
                submit,
    ...
    if let Some(prev) = keybindings.history_prev_key() {
        if prev != rustyline::KeyEvent(rustyline::KeyCode::Up, rustyline::Modifiers::NONE) {
            editor.bind_sequence(
                prev,
                rustyline::EventHandler::Simple(rustyline::Cmd::PreviousHistory),
            );
        }
    }
    if let Some(next) = keybindings.history_next_key() {
        if next != rustyline::KeyEvent(rustyline::KeyCode::Down, rustyline::Modifiers::NONE) {
            editor.bind_sequence(
                next,
                rustyline::EventHandler::Simple(rustyline::Cmd::NextHistory),
            );
        }
    }
...
/// Resolve a localized help string, falling back to the supplied English text
/// when the active locale table has no entry for `key`.
///
/// New help strings are routed through the i18n layer (`crate::tr!`) so they can
/// be translated later. Because `t()` echoes the key back when it is missing,
/// we treat an unchanged key as "untranslated" and use the English default,
/// keeping the rendered help stable until translations land.
fn help_tr(key: &str, default: &str) -> String {
...
fn print_help() {
    ...
    let title = help_tr("help.title", "Available slash commands");
    let g_session = help_tr("help.group.session", "Session");
    let g_conversation = help_tr("help.group.conversation", "Conversation");
    let g_model_mode = help_tr("help.group.model_mode", "Model & mode");
    let g_extensions = help_tr("help.group.extensions", "Extensions & prompts");
    let g_display = help_tr("help.group.display", "Display");
    let g_navigation = help_tr("help.group.navigation", "Navigation");
    let footer = help_tr(
        "help.footer",
...
{g_session}
  /help or /?            Show this command guide
  /exit or /quit         Leave the session
  /clear                 Clear the current chat history

{g_conversation}
  /compact               Compact the conversation to free context while keeping the key details
  /recipe [filepath]     Save the conversation as a reusable recipe (.yaml; defaults to ./recipe.yaml)
  /edit [text]           Compose your next message in your editor, optionally pre-filled with text
  /plan <message>        Enter plan mode: draft a plan from the conversation before acting on it
  /endplan               Leave plan mode and return to normal mode

{g_model_mode}
  /model [name]          Show the current model, or switch models for this session (same provider)
  /mode <name>           Set the working mode ({modes})

{g_extensions}
  /extension <command>   Add a stdio extension (format: ENV1=val1 command args...)
  /builtin <names>       Add builtin extensions by name (comma-separated)
  /prompts [--extension <name>]            List available prompts, optionally filtered by extension
  /prompt <name> [--info] [key=value...]   Show prompt info or run a prompt
  /skills [<name>...]    List available skills, or enable skills by name
...
fn print_editor_help() {
  ...
  Configure editor: bharatcode configure set bharatcode_prompt_editor \"vim\"
  Falls back to $VISUAL or $EDITOR if bharatcode_prompt_editor is not set.
  When bharatcode_prompt_editor is set, the editor is used for every prompt by default.
  To use inline prompts with on-demand /edit: bharatcode configure set bharatcode_prompt_editor_always false"
    );
}
