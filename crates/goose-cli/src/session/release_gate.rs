...
//! reads live in the `goose::offline` / `goose::residency` modules and the pure
...
/// `goose::offline` status; coherence reuses the pure `offline_implies_strict`
...
pub fn gather() -> ReleaseState {
    goose::offline::install_egress_guard();

    let status = goose::offline::offline_status();
    let offline_requested = status.enabled;
    let configured_residency = goose::residency::residency_mode();
