//! Generate manpages for the bharatcode CLI.
//!
...
//! offline documentation accessible via the `man` command (e.g., `man bharatcode`).
//!
//! When bharatcode is packaged for Linux distributions (deb, rpm, etc.), the generated
...
fn generate_see_also(
    ...
    if current_name != "bharatcode" {
        references.push("bharatcode".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "bharatcode" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "bharatcode" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("bharatcode-{}", subcmd_name);
