...
use bharatcode_core::agents::{Container, ExtensionLoadResult};
...
use bharatcode_core::recipe_deeplink;
use bharatcode_core::session::session_manager::SessionType;
use bharatcode_core::session::{EnabledExtensionsState, ExtensionState, Session};
use bharatcode_core::{
...
async fn start_agent(
    ...
    bharatcode_core::posthog::set_session_context("desktop", false);
    ...
    let mut session = manager
        ...
        .map_err(|err| {
            ...
            #[cfg(feature = "telemetry")]
            bharatcode_core::posthog::emit_error("session_create_failed", &err.to_string());
            ErrorResponse {
...
async fn resume_agent(
    ...
    #[cfg(feature = "telemetry")]
    bharatcode_core::posthog::set_session_context("desktop", true);
    ...
    let session = state
        ...
        .map_err(|err| {
            ...
            #[cfg(feature = "telemetry")]
            bharatcode_core::posthog::emit_error("session_resume_failed", &err.to_string());
            ErrorResponse {
...
async fn update_agent_provider(
    ...
    let config = Config::global();
    let model = match payload.model.or_else(|| config.get_bharatcode_model().ok()) {
        Some(m) => m,
        None => {
    ...
    let mut model_config =
        bharatcode_core::model_config::model_config_from_user_config(&payload.provider, &model)
            .map_err(|e| {
                (
...
async fn call_tool(
    ...
    let ctx = bharatcode_core::agents::ToolCallContext::new(payload.session_id.clone(), None, None);
