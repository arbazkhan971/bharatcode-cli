...
pub async fn session_reply(
    ...
    let is_elicitation_response = user_message.content.iter().any(|c| {
        matches!(
            c,
            bharatcode_core::conversation::message::MessageContent::ActionRequired(ar)
                if matches!(
                    ar.data,
                    bharatcode_core::conversation::message::ActionRequiredData::ElicitationResponse { .. }
                )
        )
    });
    ...
    if is_elicitation_response {
        ...
        let session_config = bharatcode_core::agents::types::SessionConfig {
            id: session_id.clone(),
