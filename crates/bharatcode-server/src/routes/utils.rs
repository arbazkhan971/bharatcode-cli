use bharatcode_core::config::declarative_providers::{load_provider, LoadedProvider};
use bharatcode_core::config::Config;
use bharatcode_core::providers::base::{ConfigKey, ProviderMetadata, ProviderType};
...
fn check_provider_configured_with_huggingface_oauth(
    ...
    if has_oauth_key {
        if let Some(entry) = bharatcode_core::config::get_provider_entry(config, &metadata.name) {
            if entry.configured {
                return true;
            }
        }
    ...
    if metadata.config_keys.is_empty() {
        ...
        if let Ok(current) = config.get_bharatcode_provider() {
