...
use bharatcode_core::providers::base::{ModelInfo, ProviderMetadata, ProviderType};
use bharatcode_core::providers::canonical::maybe_get_canonical_model;
use bharatcode_core::providers::catalog::{
    get_provider_template, get_providers_by_format, ProviderCatalogEntry, ProviderFormat,
    ProviderTemplate,
};
use bharatcode_core::providers::create_with_default_model;
...
pub async fn upsert_config(
    ...
    if query.key == "BHARATCODE_PROVIDER" {
        if let Some(name) = query.value.as_str() {
            ...
            let model = bharatcode_core::config::get_provider_entry(config, name)
                .map(|e| e.model)
                .or_else(|| config.get_bharatcode_model().ok())
                .unwrap_or_default();
            bharatcode_core::config::set_active_provider(config, name, &model)?;
            return Ok(Json(Value::String(format!("Upserted key {}", query.key))));
        }
    }
    if query.key == "BHARATCODE_MODEL" {
        if let Some(model) = query.value.as_str() {
            if let Ok(provider) = config.get_bharatcode_provider() {
                bharatcode_core::config::set_active_provider(config, &provider, model)?;
                return Ok(Json(Value::String(format!("Upserted key {}", query.key))));
            }
        }
    }
...
pub async fn remove_config(
    ...
    } else if query.key == "BHARATCODE_PROVIDER" || query.key == "active_provider" {
        config.delete("active_provider")?;
        config.delete("BHARATCODE_PROVIDER")?;
    } else if query.key == "BHARATCODE_MODEL" {
        if let Ok(provider) = config.get_bharatcode_provider() {
            bharatcode_core::config::set_active_provider(config, &provider, "")?;
        }
        config.delete("BHARATCODE_MODEL")?;
    } else {
...
fn unconfigure_provider(config: &Config, provider_name: &str) -> Result<(), ConfigError> {
    if let Some(mut entry) = bharatcode_core::config::get_provider_entry(config, provider_name) {
...
fn mark_provider_configured(config: &Config, provider_name: &str) -> Result<(), ConfigError> {
    if let Some(mut entry) = bharatcode_core::config::get_provider_entry(config, provider_name) {
        ...
        bharatcode_core::config::set_provider_entry(
            config,
            provider_name,
            &bharatcode_core::config::ProviderEntry {
                enabled: true,
                model,
                configured: true,
            },
...
pub async fn delete_provider_secret(Path(id): Path<String>) -> Result<Json<String>, ErrorResponse> {
    ...
    if let Some(provider) = parse_provider_cache_id(&id) {
        ...
        bharatcode_core::providers::cleanup_provider(provider).await?;
...
pub async fn read_config(
    ...
    if query.key == "BHARATCODE_PROVIDER" || query.key == "active_provider" {
        if let Ok(val) = config.get_bharatcode_provider() {
            return Ok(Json(ConfigValueResponse::Value(Value::String(val))));
        }
        return Ok(Json(ConfigValueResponse::Value(Value::Null)));
    }
    if query.key == "BHARATCODE_MODEL" {
        if let Ok(val) = config.get_bharatcode_model() {
            return Ok(Json(ConfigValueResponse::Value(Value::String(val))));
        }
        return Ok(Json(ConfigValueResponse::Value(Value::Null)));
    }
...
pub async fn add_extension(
    ...
    bharatcode_core::config::set_extension(ExtensionEntry {
...
pub async fn remove_extension(Path(name): Path<String>) -> Result<Json<String>, ErrorResponse> {
    ...
    bharatcode_core::config::remove_extension(&key);
...
pub async fn providers() -> Result<Json<Vec<ProviderDetails>>, ErrorResponse> {
    ...
    let providers_response: Vec<ProviderDetails> = providers
        ...
        .map(|(metadata, provider_type)| {
            ...
            let saved_model = bharatcode_core::config::get_provider_entry(config, &metadata.name)
                .map(|e| e.model)
                .filter(|m| !m.is_empty());
...
pub async fn get_slash_commands(
    ...
    for source in bharatcode_core::skills::list_installed_skills(working_dir.as_deref()) {
        commands.push(SlashCommand {
            command: source.name,
...
pub async fn upsert_permissions(
    ...
    let permission_manager = bharatcode_core::config::PermissionManager::instance();
...
pub async fn create_custom_provider(
    ...
    let config = bharatcode_core::config::declarative_providers::create_custom_provider(
        bharatcode_core::config::declarative_providers::CreateCustomProviderParams {
    ...
    bharatcode_core::providers::refresh_custom_providers().await?;
...
pub async fn remove_custom_provider(Path(id): Path<String>) -> Result<Json<String>, ErrorResponse> {
    bharatcode_core::config::declarative_providers::remove_custom_provider(&id)?;
...
pub async fn cleanup_provider_cache(
    ...
    bharatcode_core::providers::cleanup_provider(&name).await?;
...
pub async fn update_custom_provider(
    ...
    bharatcode_core::config::declarative_providers::update_custom_provider(
        bharatcode_core::config::declarative_providers::UpdateCustomProviderParams {
            id: id.clone(),
            engine: request.engine,
            display_name: request.display_name,
            api_url: request.api_url,
...
pub async fn configure_provider_oauth(
    ...
    if provider_name == huggingface_auth::HUGGINGFACE_PROVIDER_NAME {
        ...
        mark_provider_configured(bharatcode_core::config::Config::global(), &provider_name)?;
...
mod tests {
    use super::*;
    use bharatcode_core::config::ProviderEntry;
    ...
    fn unconfigure_provider_clears_structured_entry() {
        ...
        let entry = bharatcode_core::config::get_provider_entry(&config, "huggingface").unwrap();
