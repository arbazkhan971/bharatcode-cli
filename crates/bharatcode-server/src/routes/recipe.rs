...
use bharatcode_core::recipe::local_recipes;
use bharatcode_core::recipe::validate_recipe::validate_recipe_template_from_content;
use bharatcode_core::recipe::{strip_error_location, Recipe};
use bharatcode_core::{recipe_deeplink, slash_commands::recipe_slash_command};
    ...
    match recipe_result {
        Ok(mut recipe) => {
            if let Some(author_req) = request.author {
                recipe.author = Some(bharatcode_core::recipe::Author {
        ...
        Err(e) => {
            ...
            bharatcode_core::posthog::emit_error("recipe_create_failed", &e.to_string());
            let error_response = CreateRecipeResponse {
                recipe: None,
...
async fn encode_recipe(
    ...
    match recipe_deeplink::encode(&request.recipe) {
        ...
        Err(err) => {
            ...
            #[cfg(feature = "telemetry")]
            bharatcode_core::posthog::emit_error("recipe_encode_failed", &err.to_string());
...
async fn decode_recipe(
    ...
    match recipe_deeplink::decode(&request.deeplink) {
        ...
        Err(err) => {
            ...
            #[cfg(feature = "telemetry")]
            bharatcode_core::posthog::emit_error("recipe_decode_failed", &err.to_string());
            Err(StatusCode::BAD_REQUEST)
        }
    }
}
...
async fn schedule_recipe(
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            #[cfg(feature = "telemetry")]
            bharatcode_core::posthog::emit_error("recipe_schedule_failed", &e.to_string());
