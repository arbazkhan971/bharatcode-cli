...
#[cfg(feature = "local-inference")]
use bharatcode_core::dictation::providers::transcribe_local;
use bharatcode_core::dictation::providers::{
    all_providers, is_configured, transcribe_with_provider, DictationProvider,
};
#[cfg(feature = "local-inference")]
use bharatcode_core::dictation::whisper;
#[cfg(feature = "local-inference")]
use bharatcode_core::download_manager::{get_download_manager, DownloadProgress};
...
pub async fn download_model(Path(model_id): Path<String>) -> Result<StatusCode, ErrorResponse> {
    ...
    manager
        .download_model(
            ...
            model.local_path(),
            Some(Box::new(move || {
                let _ = bharatcode_core::config::Config::global()
