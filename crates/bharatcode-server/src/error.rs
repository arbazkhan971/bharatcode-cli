...
mod tests {
    ...
    fn test_env_var_conversion() {
        assert_eq!(to_env_var("type"), "BHARATCODE_PROVIDER__TYPE");
        assert_eq!(to_env_var("api_key"), "BHARATCODE_PROVIDER__API_KEY");
        assert_eq!(to_env_var("provider.host"), "BHARATCODE_PROVIDER__HOST");
        assert_eq!(
            to_env_var("provider.api_key"),
            "BHARATCODE_PROVIDER__API_KEY"
        );
    }
}
