...
use bharatcode_core::config::{paths::Paths, Config};
...
impl TunnelManager {
    ...
    fn is_tunnel_disabled() -> bool {
        if let Ok(val) = std::env::var("BHARATCODE_TUNNEL") {
    ...
    pub async fn start(&self) -> anyhow::Result<TunnelInfo> {
        if Self::is_tunnel_disabled() {
            anyhow::bail!("Tunnel is disabled via BHARATCODE_TUNNEL environment variable");
        }
