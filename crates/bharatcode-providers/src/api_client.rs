...
/// A process-wide guard invoked with the fully-resolved request URL before every
/// request is sent through the shared client.
///
/// The shared client lives in `goose-providers`, which deliberately does **not**
/// depend on the `goose` crate where the data-residency / offline egress policy
/// lives. The host crate registers a guard here so that *every* provider —
/// including declarative (JSON-defined) providers that never touch a hand-written
/// Rust module — is screened in this one central place. When no guard is
/// registered the client behaves exactly as before.
type EndpointGuard = Box<dyn Fn(&str) -> Result<()> + Send + Sync>;
static ENDPOINT_GUARD: OnceLock<EndpointGuard> = OnceLock::new();

/// Register the process-wide endpoint egress guard.
///
/// Idempotent: the first registration wins and any later call is ignored, so it
/// is safe to call from a lazily-initialised registry.
pub fn set_endpoint_guard<F>(guard: F)
where
    F: Fn(&str) -> Result<()> + Send + Sync + 'static,
{
    let _ = ENDPOINT_GUARD.set(Box::new(guard));
}

/// Run the registered egress guard (if any) against a resolved request URL.
fn screen_endpoint(url: &str) -> Result<()> {
    match ENDPOINT_GUARD.get() {
        Some(guard) => guard(url),
        None => Ok(()),
    }
}
