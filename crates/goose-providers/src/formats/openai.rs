...
pub fn create_request_with_options(
    ...
    if model_config.model_name.starts_with("o1-mini") {
        return Err(anyhow!(
            "o1-mini model is not currently supported since bharatcode uses tool calling and o1-mini does not support it. Please use o1 or o3 models instead."
        ));
    }
    ...
    // BHARATCODE_MAX_TOKENS) or a canonical model record has supplied a value.
...
mod tests {
    ...
    fn test_create_request_omits_max_tokens_when_unset() -> anyhow::Result<()> {
        ...
        // lmstudio) have no canonical record and no BHARATCODE_MAX_TOKENS, so the
...
data: [DONE]"#;
    ...
    fn test_is_openai_responses_model_matches_o_and_gpt5_families() {
        for model in [
            ...
            "gpt-5-2-pro",
            "databricks-gpt-5.4",
            "bharatcode-gpt-5.4-high",
            "headless-bharatcode-o3-mini",
        ] {
    ...
    fn test_extract_reasoning_effort_for_responses_models() {
        for (model, expected_name, expected_effort) in [
            ...
            ("bharatcode-gpt-5-high", "bharatcode-gpt-5", Some("high")),
            ("gpt-4o", "gpt-4o", None),
        ] {
