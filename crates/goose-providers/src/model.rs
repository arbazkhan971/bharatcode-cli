...
mod tests {
    ...
    mod thinking_effort_tests {
        ...
        fn xhigh_suffix_stripped_from_model_name() {
            let _guard = env_lock::lock_env([
                ("BHARATCODE_THINKING_EFFORT", Some("low")),
                ("BHARATCODE_MAX_TOKENS", None::<&str>),
                ("BHARATCODE_TEMPERATURE", None::<&str>),
                ("BHARATCODE_CONTEXT_LIMIT", None::<&str>),
                ("BHARATCODE_TOOLSHIM", None::<&str>),
    ...
    mod is_openai_reasoning_model {
        ...
        const ENV_LOCK_KEYS: [(&str, Option<&str>); 5] = [
            ("BHARATCODE_MAX_TOKENS", None),
            ("BHARATCODE_TEMPERATURE", None),
            ("BHARATCODE_CONTEXT_LIMIT", None),
            ("BHARATCODE_TOOLSHIM", None),
            ("BHARATCODE_TOOLSHIM_OLLAMA_MODEL", None),
        ];
        ...
        fn goose_prefixed_reasoning_models() {
            let _guard = env_lock::lock_env(ENV_LOCK_KEYS);
            assert!(ModelConfig::new_or_fail("bharatcode-o3-mini").is_openai_reasoning_model());
            assert!(ModelConfig::new_or_fail("bharatcode-o4-mini").is_openai_reasoning_model());
            assert!(ModelConfig::new_or_fail("bharatcode-gpt-5").is_openai_reasoning_model());
        }
        ...
        fn non_reasoning_models() {
            ...
            assert!(
                !ModelConfig::new_or_fail("bharatcode-claude-sonnet-4").is_openai_reasoning_model()
            );
