...
async fn run() -> Result<()> {
    if let Err(e) = bharatcode_cli::logging::setup_logging(None) {
...
fn main() -> Result<()> {
    ...
    let handle = std::thread::Builder::new()
        .name("bharatcode-cli-main".to_string())
        ...
        .map_err(|e| anyhow::anyhow!("Failed to spawn bharatcode-cli main thread: {}", e))?;

    handle
        .join()
        .map_err(|_| anyhow::anyhow!("bharatcode-cli main thread panicked"))?
}
