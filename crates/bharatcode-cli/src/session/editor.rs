...
/// Checks BHARATCODE_PROMPT_EDITOR, then $VISUAL, then $EDITOR.
pub fn resolve_editor_command() -> Option<String> {
    let config = Config::global();
    let config_editor = config.get_bharatcode_prompt_editor().ok().flatten();
...
fn build_template(messages: &[&str], prefill: Option<&str>) -> String {
    let mut content = String::from("# BharatCode Prompt Editor\n\n");
...
pub fn get_editor_input(
    ...
    let symlink_path = PathBuf::from(".bharatcode_prompt_temp.md");
...
mod tests {
    ...
    fn test_extract_user_input_with_editor_output() {
        let content = r#"# BharatCode Prompt Editor
...
"#;
    ...
    fn test_create_temp_file_with_messages() {
        ...
        assert!(path.exists());
        assert!(path.to_str().unwrap().contains("bharatcode_prompt_"));
        assert!(path.to_str().unwrap().ends_with(".md"));

        let content = fs::read_to_string(path).unwrap();
        assert!(content.contains("# BharatCode Prompt Editor"));
    ...
    fn test_create_temp_file_with_prefix_suffix() {
        let temp_file = Builder::new()
            .prefix("bharatcode_test_")
            .suffix(".md")
            .tempfile()
            .unwrap();

        let name = temp_file.path().file_name().unwrap().to_str().unwrap();
        assert!(name.starts_with("bharatcode_test_"));
...
"#;
    ...
    fn test_tempfile_cleanup() {
        let path = {
            let temp_file = Builder::new()
                .prefix("bharatcode_cleanup_test_")
                .tempfile()
                .unwrap();
    ...
    fn test_build_template_no_prefill_no_messages() {
        ...
        assert_eq!(content, "# BharatCode Prompt Editor\n\n# Your prompt:\n\n");
    }
