...
use bharatcode_core::permission::Permission;
use bharatcode_core::permission::PermissionConfirmation;
...
const BHARATCODE_PLANNER_CONTEXT_LIMIT: &str = "BHARATCODE_PLANNER_CONTEXT_LIMIT";
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        let mut parts = bharatcode_core::utils::split_command_args(extension_command)?;
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: bharatcode_core::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            timeout: Some(bharatcode_core::config::DEFAULT_EXTENSION_TIMEOUT),
            cwd: None,
            bundled: None,
    ...
    async fn run_interactive(&mut self, prompt: Option<String>) -> Result<()> {
        ...
        loop {
            ...
            let input =
                input::get_input(&mut editor, &self.keybindings, Some(&conversation_strings))?;
                    ...
                    None => {
                        output::render_error(
                            "No editor found. Set one with:\n  \
                                 bharatcode configure set bharatcode_prompt_editor \"vim\"\n  \
    ...
    async fn handle_goose_mode(&self, mode: &str) -> Result<()> {
        ...
        output::goose_mode_message(&format!("BharatCode mode set to '{mode}'"));
        Ok(())
    }
    ...
    async fn handle_model(&self, model: Option<&str>) -> Result<()> {
        ...
        let configured_effort = Config::global().get_bharatcode_thinking_effort();
    ...
    async fn handle_clear(&mut self) -> Result<()> {
        ...
        if let Err(e) = self
            ...
            .usage(bharatcode_providers::conversation::token_usage::Usage::new(
                Some(0),
                Some(0),
                Some(0),
            ))
        ...
        match planner_response_type {
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    let config = Config::global();
                    let curr_goose_mode = config.get_bharatcode_mode().unwrap_or_default();
                    if curr_goose_mode != GooseMode::Auto {
                        config.set_bharatcode_mode(GooseMode::Auto).unwrap();
                    }
                    ...
                    if curr_goose_mode != GooseMode::Auto {
                        config.set_bharatcode_mode(curr_goose_mode)?;
                    }
                } else {
    ...
    async fn process_agent_response(
        ...
        // deny mode, refuse to start further model turns once it is exceeded.
        {
            let session_usd = self
                .get_session()
                .await
                .ok()
                .and_then(|s| s.accumulated_cost)
                .unwrap_or(0.0);
            let decision = crate::commands::budget::gate_turn(session_usd).await;
            if let Some(msg) = decision.message() {
                if decision.is_blocked() {
                    // A deny-mode budget block must stay observable even in
                    // JSON modes; otherwise scripted consumers see an empty,
                    // silent "success" for a turn that never ran.
                    if is_stream_json_mode {
                        ...
                        let json_output = JsonOutput {
                            ...
                            metadata: JsonMetadata {
                                ...
                                input_tokens: None,
                                output_tokens: None,
                                status: "budget_blocked".to_string(),
                                error: Some(msg.to_string()),
                            },
                        };
                        println!("{}", serde_json::to_string_pretty(&json_output)?);
                    } else {
                        output::render_text(msg, Some(Color::Red), false);
                    }
                } else if !is_json_mode && !is_stream_json_mode {
                    output::render_text(msg, Some(Color::Yellow), false);
                }
            }
            if decision.is_blocked() {
                return Ok(());
                                ...
                                let permission = if interactive {
                                    ...
                                    if goose_mode == GooseMode::Approve || goose_mode == GooseMode::SmartApprove {
                                        ...
                                        return Err(anyhow::anyhow!(
                                            "Tool approval required in non-interactive mode with BharatCodeMode::{goose_mode}. \
                                             This is an invalid configuration — Approve/SmartApprove modes require an \
                                             interactive terminal. Use BharatCodeMode::Auto for headless sessions."
                                        ));
                                    }
                                    tracing::warn!(
    ...
    pub async fn get_session(&self) -> Result<bharatcode_core::session::Session> {
        self.agent
            .config
            .session_manager
            .get_session(&self.session_id, false)
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let show_cost = config
            .get_param::<bool>("BHARATCODE_CLI_SHOW_COST")
            .unwrap_or(false);
    ...
    fn save_recipe(
        &self,
        recipe: &bharatcode_core::recipe::Recipe,
        filepath_str: &str,
...
fn prompt_tool_confirmation(security_prompt: &Option<String>) -> Result<Permission> {
    ...
    let prompt = if let Some(security_message) = security_prompt {
        ...
        "BharatCode would like to call the above tool, do you allow?".to_string()
    };
                ...
                let formatted = match notification_type {
                    ...
                    Some("response_generated") => {
                        ...
                        let min_priority = config
                            .get_param::<f32>("BHARATCODE_CLI_MIN_PRIORITY")
                            .ok()
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<bharatcode_providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                bharatcode_providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
async fn get_reasoner() -> Result<Arc<dyn Provider>, anyhow::Error> {
    ...
    let provider = if let Ok(provider) = config.get_param::<String>("BHARATCODE_PLANNER_PROVIDER") {
        provider
    } else {
        println!("WARNING: BHARATCODE_PLANNER_PROVIDER not found. Using default provider...");
        config
            .get_bharatcode_provider()
            .expect("No provider configured. Run 'bharatcode configure' first")
    };

    // Try planner-specific model first, fall back to default model
    let model = if let Ok(model) = config.get_param::<String>("BHARATCODE_PLANNER_MODEL") {
        model
    } else {
        println!("WARNING: BHARATCODE_PLANNER_MODEL not found. Using default model...");
        config
            .get_bharatcode_model()
            .expect("No model configured. Run 'bharatcode configure' first")
    };

    let planner_context_limit = match env::var(BHARATCODE_PLANNER_CONTEXT_LIMIT)
        .ok()
        .map(|v| v.parse::<usize>())
    {
        Some(Ok(n)) if n >= 4096 => Some(n),
        Some(Ok(_)) => anyhow::bail!("{} must be at least 4096", BHARATCODE_PLANNER_CONTEXT_LIMIT),
        Some(Err(e)) => anyhow::bail!("{}: {}", BHARATCODE_PLANNER_CONTEXT_LIMIT, e),
        None => None,
    };
...
fn build_switched_model_config(
    provider_name: &str,
    model_name: &str,
    current_model_config: &bharatcode_providers::model::ModelConfig,
) -> Result<bharatcode_providers::model::ModelConfig> {
...
mod tests {
    use super::*;
    use bharatcode_core::agents::extension::Envs;
    use bharatcode_core::config::ExtensionConfig;
    ...
    fn test_build_switched_model_config_rebuilds_target_model_settings() {
        let _guard = env_lock::lock_env([
            ("BHARATCODE_MAX_TOKENS", None::<&str>),
            ("BHARATCODE_TEMPERATURE", None::<&str>),
            ("BHARATCODE_CONTEXT_LIMIT", None::<&str>),
            ("BHARATCODE_TOOLSHIM", None::<&str>),
            ("BHARATCODE_TOOLSHIM_OLLAMA_MODEL", None::<&str>),
        ]);

        let current_model_config = bharatcode_providers::model::ModelConfig {
        ...
        let expected = bharatcode_providers::model::ModelConfig::new_or_fail("gpt-5.4")
    ...
    fn test_build_switched_model_config_detects_effort_suffix_change() {
        ...
        let current = bharatcode_providers::model::ModelConfig::new_or_fail("gpt-5.4-high")
            .with_canonical_limits("openai");
        assert_eq!(current.model_name, "gpt-5.4");
        assert_eq!(
            current.thinking_effort(),
            Some(bharatcode_providers::thinking::ThinkingEffort::High)
        );
    ...
    fn test_split_command_args_windows_paths() {
        assert_eq!(
            bharatcode_core::utils::split_command_args(r"C:\tools\mcp.exe --arg value").unwrap(),
    ...
    fn test_split_command_args_unmatched_quote() {
        assert!(bharatcode_core::utils::split_command_args(r#""unmatched"#).is_err());
    }
