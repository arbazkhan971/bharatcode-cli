    ...
    fn as_str(&self) -> String {
        match self {
            Theme::Light => Config::global()
                .get_param::<String>("BHARATCODE_CLI_LIGHT_THEME")
                .unwrap_or(DEFAULT_CLI_LIGHT_THEME.to_string()),
            Theme::Dark => Config::global()
                .get_param::<String>("BHARATCODE_CLI_DARK_THEME")
...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("BHARATCODE_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("BHARATCODE_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
                    .unwrap_or(Theme::Ansi)
            )
    );
    static SHOW_FULL_TOOL_OUTPUT: RefCell<bool> = RefCell::new(
        Config::global().get_param::<bool>("BHARATCODE_SHOW_FULL_OUTPUT").unwrap_or(false)
    );
}
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("BHARATCODE_CLI_THEME", theme.as_config_string())
    ...
    if let Err(e) = config.set_param("BHARATCODE_CLI_THEME", theme_str) {
...
pub fn run_status_hook(status: &str) {
    if let Ok(hook) = Config::global().get_param::<String>("BHARATCODE_STATUS_HOOK") {
...
fn should_show_thinking() -> bool {
    Config::global()
        .get_param::<bool>("BHARATCODE_CLI_SHOW_THINKING")
        .unwrap_or(false)
...
pub fn display_cost_usage(provider: &str, model: &str, usage: &Usage) {
    if let Some(cost) = estimate_cost_usd(provider, model, usage) {
        ...
        // BharatCode INR layer: surface a compact rupee figure alongside USD in
        // the session footer, using the configured USD->INR rate.
