...
//! You can set the BHARATCODE_TEST_PROVIDER to just run a specific provider.
    ...
    async fn test_what_is_your_name() -> Result<()> {
        run_scenario(
            ...
            |result| {
                ...
                assert!(
                    result.last_message()?.to_lowercase().contains("bharatcode"),
                    "Response should contain 'bharatcode': {}",
                    result.last_message()?
                );
                Ok(())
            },
