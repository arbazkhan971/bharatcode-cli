...
use anyhow::Result;
use bharatcode_core::agents::{Agent, AgentConfig, GoosePlatform};
use bharatcode_core::config::permission::PermissionManager;
use bharatcode_core::config::GooseMode;
use bharatcode_core::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("BHARATCODE_TEST_PROVIDER") {
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    bharatcode_core::agents::moim::SKIP.with(|f| f.set(true));
    ...
    agent
        .update_provider(
            provider_arc as Arc<dyn bharatcode_core::providers::base::Provider>,
            &session.id,
        )
        .await?;
