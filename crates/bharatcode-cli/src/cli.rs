...
use bharatcode_core::posthog::get_telemetry_choice;
...
use bharatcode_core::agents::Container;
...
async fn handle_serve_command(
    ...
    use bharatcode_core::acp::transport::create_router;
    ...
    match (
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    bharatcode_core::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
...
async fn handle_local_models_command(command: LocalModelsCommand) -> Result<()> {
    use bharatcode_core::providers::local_inference::hf_models;
    use bharatcode_core::providers::local_inference::local_model_registry::get_registry;
    ...
    match command {
        ...
        LocalModelsCommand::Download { spec } => {
            ...
            let manager = bharatcode_core::download_manager::get_download_manager();
...
pub async fn cli() -> anyhow::Result<()> {
    register_builtin_extensions(bharatcode_mcp::BUILTIN_EXTENSIONS.clone());
    ...
    match cli.command {
        ...
        Some(Command::ValidateExtensions { file }) => {
            use bharatcode_core::agents::validate_extensions::validate_bundled_extensions;
