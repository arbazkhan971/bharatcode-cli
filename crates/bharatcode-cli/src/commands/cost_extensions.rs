...
//!     [`bharatcode_core::plugins`] discovery helpers; nothing is installed, mutated, or
...
///
/// [`bharatcode_core::plugins::installed_plugin_skill_dirs`] returns directories nested
...
fn collect_extension_names() -> Vec<String> {
    ...
    for skill_dir in bharatcode_core::plugins::installed_plugin_skill_dirs() {
        if let Some(name) = plugin_name_from_skill_dir(&skill_dir, &plugins_dir) {
            names.insert(name);
        }
    }

    for ext in bharatcode_core::plugins::mcp_servers::enabled_plugin_mcp_servers(None) {
        let name = ext.name();
        let name = name.trim();
        if !name.is_empty() {
