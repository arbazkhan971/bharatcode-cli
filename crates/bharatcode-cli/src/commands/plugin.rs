...
pub fn handle_plugin_install(url: &str, auto_update: bool) -> Result<()> {
    let install = bharatcode_core::plugins::install_plugin_with_options(
        url,
        bharatcode_core::plugins::PluginInstallOptions { auto_update },
    )?;
...
pub fn handle_plugin_update(name: &str) -> Result<()> {
    let install = bharatcode_core::plugins::update_plugin(name)?;
...
fn print_plugin_install(install: &bharatcode_core::plugins::PluginInstall) {
