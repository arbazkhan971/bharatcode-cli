...
//! [`bharatcode_core::memory_store::is_enabled`]) so a bare `1` is honoured rather than
...
/// is honoured (mirrors [`bharatcode_core::memory_store::is_enabled`], which avoids the
