...
async fn print_settings_summary() {
    ...
    let residency = read_setting(config, &[bharatcode_core::residency::RESIDENCY_MODE_KEY])
    ...
    let offline = if bharatcode_core::offline::is_offline() {
        label("doctor.on", "on")
    } else {
        label("doctor.off", "off")
    };
...
fn telemetry_enabled(config: &bharatcode_core::config::Config) -> bool {
...
fn read_setting(config: &bharatcode_core::config::Config, keys: &[&str]) -> Option<String> {
    for &key in keys {
...
fn ollama_host_port(config: &bharatcode_core::config::Config) -> String {
    let raw = config
