...
pub async fn handle_session_export(
    ...
    if nostr {
        return Err(anyhow::anyhow!(
            "bharatcode was not built with nostr support"
        ));
    }
...
pub async fn handle_session_import(input: String, nostr: bool) -> Result<()> {
    let json = if nostr || input.starts_with("bharatcode://sessions/nostr") {
        #[cfg(feature = "nostr")]
        {
    ...
    let format = bharatcode_core::session::import_formats::detect_format(&json);
    let label = match format {
        bharatcode_core::session::import_formats::ImportFormat::Goose => "bharatcode",
        bharatcode_core::session::import_formats::ImportFormat::ClaudeCode => "Claude Code",
        bharatcode_core::session::import_formats::ImportFormat::Codex => "Codex",
        bharatcode_core::session::import_formats::ImportFormat::Pi => "Pi",
    };
...
fn export_session_to_markdown(
    messages: Vec<bharatcode_core::conversation::message::Message>,
    session_name: &String,
) -> String {
    ...
    for message in &messages {
        ...
        let is_only_tool_response = message.role == rmcp::model::Role::User
            && message.content.iter().all(|content| {
                matches!(
                    content,
                    bharatcode_core::conversation::message::MessageContent::ToolResponse(_)
                )
            });
        ...
        if message.content.iter().any(|content| {
            matches!(
                content,
                bharatcode_core::conversation::message::MessageContent::ToolRequest(_)
            )
        }) {
