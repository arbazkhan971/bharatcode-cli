...
fn asset_name() -> &'static str {
    ...
    {
        "bharatcode-x86_64-apple-darwin.tar.bz2"
    }
    ...
    {
        "bharatcode-aarch64-unknown-linux-gnu.tar.bz2"
    }
    ...
    {
        "bharatcode-aarch64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", feature = "cuda"))]
    {
        "bharatcode-x86_64-pc-windows-msvc-cuda.zip"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", not(feature = "cuda")))]
    {
        "bharatcode-x86_64-pc-windows-msvc.zip"
    }
}
...
async fn fetch_attestations(digest: &str, token: Option<&str>) -> Result<Vec<serde_json::Value>> {
    ...
    let mut req = client
        ...
        .header("User-Agent", "bharatcode-cli");
...
/// Update the bharatcode binary to the latest release.
///
...
pub async fn update(canary: bool, reconfigure: bool) -> Result<()> {
    ...
    #[cfg(not(feature = "disable-update"))]
    {
        ...
        if reconfigure {
            println!("Running bharatcode configure...");
            let status = Command::new(current_exe)
                .arg("configure")
                .status()
                .context("Failed to run bharatcode configure")?;
            if !status.success() {
                eprintln!("Warning: bharatcode configure exited with {status}");
            }
        }
...
///   1. A `bharatcode-package/` subdirectory (Windows releases)
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if old_exe.exists() {
            fs::remove_file(&old_exe).with_context(|| {
                format!(
                    "Failed to remove old backup {}. Is another bharatcode process running?",
                    old_exe.display()
                )
            })?;
        }
        ...
        fs::rename(current_exe, &old_exe).with_context(|| {
            format!(
                "Failed to rename running binary to {}. Try closing BharatCode Desktop if it's open.",
                old_exe.display()
            )
        })?;
...
mod tests {
    ...
    fn test_asset_name_valid() {
        ...
        assert!(name.starts_with("bharatcode-"));
        #[cfg(target_os = "windows")]
    ...
    fn test_binary_name() {
        ...
        #[cfg(target_os = "windows")]
        assert_eq!(name, "bharatcode.exe");
        #[cfg(not(target_os = "windows"))]
        assert_eq!(name, "bharatcode");
    }

    #[test]
    fn test_find_binary_in_package_subdir() {
        let tmp = tempdir().unwrap();
        let pkg = tmp.path().join("bharatcode-package");
    ...
    fn test_replace_binary_basic() {
        let tmp = tempdir().unwrap();
        let new_bin = tmp.path().join("new_bharatcode");
        let current = tmp.path().join("current_bharatcode");
    ...
    fn test_replace_binary_windows_rename_away() {
        let tmp = tempdir().unwrap();
        let current = tmp.path().join("bharatcode.exe");
        let new_bin = tmp.path().join("new_bharatcode.exe");
    ...
    fn test_extract_zip_with_package_dir() {
        ...
        let mut buf = Vec::new();
        {
            ...
            writer
                .add_directory("bharatcode-package/", options)
                .unwrap();
            writer
                .start_file("bharatcode-package/bharatcode.exe", options)
                .unwrap();
            writer.write_all(b"fake bharatcode binary").unwrap();
            writer
                .start_file("bharatcode-package/libtest.dll", options)
                .unwrap();
        ...
        let binary = find_binary(tmp.path(), "bharatcode.exe");
    ...
    fn test_validate_entry_path_accepts_safe_paths() {
        assert!(validate_entry_path(Path::new("bharatcode")).is_ok());
        assert!(validate_entry_path(Path::new("bharatcode-package/bharatcode")).is_ok());
    ...
    fn test_extract_tar_bz2_safe_archive() {
        ...
        let mut builder_buf = Vec::new();
        {
            ...
            let data = b"bharatcode binary content";
            let mut header = tar::Header::new_gnu();
            header.set_size(data.len() as u64);
            header.set_mode(0o755);
            header.set_cksum();
            builder
                .append_data(&mut header, "bharatcode-package/bharatcode", &data[..])
                .unwrap();
        ...
        let extracted = tmp.path().join("bharatcode-package/bharatcode");
        assert!(extracted.exists());
        assert_eq!(
            fs::read_to_string(extracted).unwrap(),
            "bharatcode binary content"
        );
    }
