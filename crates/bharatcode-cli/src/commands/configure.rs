...
pub async fn handle_configure() -> anyhow::Result<()> {
    if !std::io::stdin().is_terminal() {
        anyhow::bail!(
            "bharatcode configure requires an interactive terminal.\n\
             If you installed via 'curl ... | bash', run 'bharatcode configure' separately after installation."
        );
    }
...
pub fn configure_telemetry_consent_dialog() -> anyhow::Result<bool> {
    ...
    println!();
    println!("{}", style("Help improve bharatcode").bold());
    println!();
    println!(
        "{}",
        style("Would you like to help improve bharatcode by sharing anonymous usage data?").dim()
    );
    println!(
        "{}",
        style(
            "This helps us understand how bharatcode is used and identify areas for improvement."
        )
        .dim()
    );
    println!();
    ...
    println!(
        "{}",
        style("  • bharatcode version and install method").dim()
    );
    ...
    println!(
        "{}",
        style("or any personal data. You can change this anytime with 'bharatcode configure'.")
            .dim()
    );
    println!();

    let enabled = cliclack::confirm("Share anonymous usage data to help improve bharatcode?")
        .initial_value(true)
        .interact()?;
    ...
    if enabled {
        let _ = cliclack::log::success("Thank you for helping improve bharatcode!");
    } else {
...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    ...
    let setup_method = cliclack::select("How would you like to set up your provider?")
        .item(
            ...
            "Run models on your machine at localhost:11434, no account or API key",
        )
        .item(
            "presets",
            crate::tr!("configure.presets_label"),
            crate::tr!("configure.presets_hint"),
        )
        .item(
            "openrouter",
            "OpenRouter Login",
    ...
    match setup_method {
        ...
        "presets" => handle_india_presets_setup(config).await,
        "local" | "manual" => handle_manual_provider_setup(config).await,
        _ => unreachable!(),
    }
    Ok(())
}
...
/// Onboard via a curated India / open-weight preset.
///
/// The user picks a recommended model; we resolve its provider metadata,
/// configure any required API keys (local Ollama presets need none), test the
/// connection, then persist it as the active provider. The preset's model id is
/// used directly, so no network model-listing call is needed.
async fn handle_india_presets_setup(config: &Config) {
    match configure_preset_dialog(config).await {
        Ok(true) => {
            println!(
                "\n  {}: Run '{}' again to adjust your config or add extensions",
                style("Tip").green().italic(),
...
async fn configure_preset_dialog(config: &Config) -> anyhow::Result<bool> {
    use crate::commands::presets::india_presets;

    let presets = india_presets();
    let mut select = cliclack::select(crate::tr!("presets.select_prompt"));
    for (idx, preset) in presets.iter().enumerate() {
        select = select.item(idx, preset.label, preset.note);
    }
    let chosen = select.interact()?;
    let preset = &presets[chosen];

    let available_providers = providers().await;
    let provider_meta = match available_providers
        .iter()
        .find(|(p, _)| p.name == preset.provider)
    {
        Some((meta, _)) => meta,
        None => {
            cliclack::outro(
    ...
    {
        if !configure_single_key(config, preset.provider, &provider_meta.display_name, key).await? {
            return Ok(false);
        }
    }
    ...
    let toolshim_enabled = std::env::var("BHARATCODE_TOOLSHIM")
        .map(|val| val == "1" || val.to_lowercase() == "true")
        .unwrap_or(false);
    let toolshim_model = std::env::var("BHARATCODE_TOOLSHIM_OLLAMA_MODEL").ok();
    ...
    {
        Ok(()) => {
            bharatcode_core::config::set_active_provider(config, preset.provider, preset.model_id)?;
...
async fn handle_existing_config() -> anyhow::Result<()> {
    ...
    let action = cliclack::select("What would you like to configure?")
        ...
        .item(
            "settings",
            "bharatcode settings",
            "Set the bharatcode mode, Tool Output, Tool Permissions, Experiment, bharatcode recipe github repo and more",
        )
        .interact()?;
...
fn select_model_from_list(
    models: &[String],
    provider_meta: &bharatcode_core::providers::base::ProviderMetadata,
    ...
    match config.set_secret(key_name, &value) {
        ...
        Err(e) => {
            cliclack::outro(style(format!(
                "Failed to store {} securely: {}. Please ensure your system's secure storage is accessible. Alternatively you can run with BHARATCODE_DISABLE_KEYRING=true or set the key in your environment variables",
                key_name, e
            )).on_red().white())?;
            Ok(false)
        }
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    // Get current default provider if it exists, else preselect the local default
    let current_provider: Option<String> = config.get_bharatcode_provider().ok();
    let default_provider = current_provider.unwrap_or_else(|| "ollama".to_string());
    ...
    let temp_model_config = bharatcode_core::model_config::model_config_from_user_config(
        provider_name,
    ...
    let model: String = match models_res {
        ...
        Ok(_) => {
            let default_model =
                std::env::var("BHARATCODE_MODEL").unwrap_or(provider_meta.default_model.clone());
...

    {
        let supports_thinking = match temp_provider.fetch_model_info(&model).await {
            ...
            Err(_) => bharatcode_providers::model::ModelConfig::new(&model)
        ...
        if supports_thinking {
            ...
            config.set_bharatcode_thinking_effort(effort)?;
        }
    }
    ...
    {
        Ok(()) => {
            bharatcode_core::config::set_active_provider(config, provider_name, &model)?;
            print_config_file_saved()?;
            Ok(true)
        }
        Err(e) => {
...
/// Configure extensions that can be used with bharatcode
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&bharatcode_core::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
fn configure_stdio_extension() -> anyhow::Result<()> {
    ...
    let mut parts = bharatcode_core::utils::split_command_args(&command_str)?;
...
pub fn configure_extensions_dialog() -> anyhow::Result<()> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with bharatcode",
        )
        .item(
            "stdio",
...
pub async fn configure_settings_dialog() -> anyhow::Result<()> {
    ...
    let setting_type = setting_select
        ...
        .item(
            "recipe",
            "bharatcode recipe github repo",
            "bharatcode will pull recipes from this repo if not found locally.",
        )
        .interact()?;
...
pub fn configure_goose_mode_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("BHARATCODE_MODE").is_ok() {
        let _ = cliclack::log::info(
            "Notice: BHARATCODE_MODE environment variable is set and will override the configuration here.",
        );
    }

    let mode = cliclack::select("Which bharatcode mode would you like to configure?")
        .item(
            GooseMode::Auto,
            "Auto Mode",
    ...
    config.set_bharatcode_mode(mode)?;
    let msg = match mode {
...
pub fn configure_telemetry_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("BHARATCODE_TELEMETRY_OFF").is_ok() {
        let _ = cliclack::log::info(
            "Notice: BHARATCODE_TELEMETRY_OFF environment variable is set and will override the configuration here.",
        );
    }
    ...
    if enabled {
        cliclack::outro("Telemetry enabled - thank you for helping improve bharatcode!")?;
    } else {
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("BHARATCODE_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info(
            "Notice: BHARATCODE_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.",
        );
    }
    ...
    match tool_log_level {
        "high" => {
            config.set_param("BHARATCODE_CLI_MIN_PRIORITY", 0.8)?;
            cliclack::outro("Showing tool output of high importance only.")?;
        }
        "medium" => {
            config.set_param("BHARATCODE_CLI_MIN_PRIORITY", 0.2)?;
            cliclack::outro("Showing tool output of medium importance.")?;
        }
        "all" => {
            config.set_param("BHARATCODE_CLI_MIN_PRIORITY", 0.0)?;
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("BHARATCODE_DISABLE_KEYRING").is_ok() {
        let _ = cliclack::log::info(
            "Notice: BHARATCODE_DISABLE_KEYRING environment variable is set and will override the configuration here.",
        );
    }

    let currently_disabled = config
        .get_param::<String>("BHARATCODE_DISABLE_KEYRING")
        .is_ok();
    ...
    match storage_option {
        "keyring" => {
            ...
            config.set_param("BHARATCODE_DISABLE_KEYRING", Value::String("".to_string()))?;
            cliclack::outro("Secret storage set to system keyring (secure)")?;
            let _ = cliclack::log::info(
                "You may need to restart bharatcode for this change to take effect",
            );
        }
        "file" => {
            // Set the disable flag to use file storage
            config.set_param(
                "BHARATCODE_DISABLE_KEYRING",
...
/// Configure experiment features that can be used with bharatcode
...
fn configure_recipe_dialog() -> anyhow::Result<()> {
    let key_name = BHARATCODE_RECIPE_GITHUB_REPO_CONFIG_KEY;
    let config = Config::global();
    ...
    let mut recipe_repo_input = cliclack::input(
        "Enter your bharatcode recipe GitHub repo (owner/repo): eg: my_org/bharatcode-recipes",
    )
    .required(false);
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    let current_max_turns: u32 = config.get_param("BHARATCODE_MAX_TURNS").unwrap_or(1000);
    ...
    config.set_param("BHARATCODE_MAX_TURNS", max_turns)?;

    cliclack::outro(format!(
        "Set maximum turns to {} - bharatcode will ask for input after {} consecutive actions",
        max_turns, max_turns
    ))?;
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use bharatcode_core::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
        ...
        Ok(provider) => {
            ...
            let test_result = provider
                .complete(
                    ...
                    "",
                    "You are bharatcode, an AI assistant.",
            ...
            match test_result {
                Ok(_) => {
                    ...
                    cliclack::outro("OpenRouter setup complete! You can now use bharatcode.")?;
                }
                Err(e) => {
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    cliclack::outro(
                        "Tetrate Agent Router Service setup complete! You can now use bharatcode.",
                    )?;
                }
                Err(e) => {
                    eprintln!("⚠️  Configuration test failed: {}", e);
...
fn add_provider() -> anyhow::Result<()> {
    ...
    if !provider_config.models.is_empty() {
        ...
        if let Ok(model) = cliclack::select("Which model should be the default?")
            ...
            .interact()
        {
            config.set_bharatcode_provider(&provider_config.name)?;
            config.set_bharatcode_model(model)?;
        }
    }
...
async fn remove_provider() -> anyhow::Result<()> {
    ...
    if let Err(e) = bharatcode_core::providers::cleanup_provider(selected_id).await {
