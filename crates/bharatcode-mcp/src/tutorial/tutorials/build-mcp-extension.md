...
Instruct users to start a bharatcode session with their extension.

**Important**: You cannot start the bharatcode session for them, as it is interactive. You will have to let them
know to start it in a terminal. Make sure you include instructions on how to set up the environment

```bash
# Python example
bharatcode session --with-extension "python server.py"

# TypeScript example
bharatcode session --with-extension "node server.js"

# Kotlin example
bharatcode session --with-extension "java -jar build/libs/extension.jar"
```
...
Note:
You can run a feedback loop using a headless bharatcode session, however if the process hangs you get into a stuck action.
Ask the user if they want you to do that, and let them know they will manually need to kill any stuck processes.

```bash
# Python example
bharatcode run --with-extension "python server.py" --text "EXAMPLE PROMPT HERE"

# TypeScript example
bharatcode run --with-extension "node server.js" --text "EXAMPLE PROMPT HERE"

# Kotlin example
bharatcode run --with-extension "java -jar build/libs/extension.jar" --text "EXAMPLE PROMPT HERE"
```
...
- For tools, they should ask bharatcode to use the tool directly
- For resources, they should ask bharatcode to access the relevant data
...
1. First, check if there are any immediate error messages in the bharatcode session
