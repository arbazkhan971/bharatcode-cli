...
mod tests {
    ...
    mod retry_tests {
        use super::*;
        use bharatcode_core::agents::types::{RetryConfig, SuccessCheck};

        #[tokio::test]
        async fn test_retry_success_check_execution() -> Result<()> {
            use bharatcode_core::agents::retry::execute_success_checks;
    ...
    mod max_turns_tests {
        use super::*;
        use async_trait::async_trait;
        use bharatcode_core::agents::SessionConfig;
        use bharatcode_core::config::GooseMode;
        ...
        impl ProviderDef for MockToolProvider {
            ...
            fn from_env(
                _model: ModelConfig,
                _extensions: Vec<bharatcode_core::config::ExtensionConfig>,
                _tls_config: Option<bharatcode_core::providers::api_client::TlsConfig>,
            ...
            while let Some(response_result) = reply_stream.next().await {
                match response_result {
                    Ok(AgentEvent::Message(response)) => {
                        ...
                        {
                            if let bharatcode_core::conversation::message::ActionRequiredData::ToolConfirmation { id, .. } = &action.data {
                                agent.handle_confirmation(
                                    id.clone(),
                                    bharatcode_core::permission::PermissionConfirmation {
                                        principal_type: bharatcode_core::permission::permission_confirmation::PrincipalType::Tool,
                                        permission: bharatcode_core::permission::Permission::AllowOnce,
                                    }
                                ).await;
                            }
                        }
    ...
    mod tool_pair_summarization_tests {
        ...
        use async_trait::async_trait;
        use bharatcode_core::agents::SessionConfig;
        use bharatcode_core::config::base::Config;
        use bharatcode_core::config::GooseMode;
        ...
        async fn test_batch_summarization_preserves_all_summaries() -> Result<()> {
            ...
            Config::global()
                .set_param("BHARATCODE_TOOL_CALL_CUTOFF", 2)
                .unwrap();
            ...
            Config::global()
                .delete("BHARATCODE_TOOL_CALL_CUTOFF")
                .unwrap();
    ...
    mod extension_manager_tests {
        use super::*;
        use bharatcode_core::agents::extension::ExtensionConfig;
        use bharatcode_core::agents::platform_extensions::{
    ...
    mod streaming_persistence_tests {
        use super::*;
        use async_trait::async_trait;
        use bharatcode_core::agents::{AgentConfig, SessionConfig};
        use bharatcode_core::config::permission::PermissionManager;
        use bharatcode_core::config::GooseMode;
        use bharatcode_core::conversation::message::Message;
    ...
    mod frontend_extension_tests {
        use super::*;
        use bharatcode_core::agents::{AgentConfig, ExtensionConfig};
        use bharatcode_core::config::permission::PermissionManager;
        use bharatcode_core::config::GooseMode;
        use bharatcode_core::session::session_manager::SessionType;
        use bharatcode_core::session::{
