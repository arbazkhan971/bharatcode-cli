...
const DEFAULT_ACP_TEST_CONFIG: &str =
    "BHARATCODE_MODEL: gpt-4o\nBHARATCODE_PROVIDER: openai\nBHARATCODE_DISABLE_KEYRING: true\n";
...
fn write_acp_global_config(contents: &str) -> PathBuf {
    std::env::set_var("BHARATCODE_PATH_ROOT", ACP_CONFIG_ROOT.path());
    std::env::set_var("BHARATCODE_DISABLE_KEYRING", "1");
    let config_dir = bharatcode_core::config::paths::Paths::config_dir();
    std::fs::create_dir_all(&config_dir).unwrap();
    let mut contents = contents.to_string();
    if !contents.contains("BHARATCODE_DISABLE_KEYRING") {
        contents.push_str("BHARATCODE_DISABLE_KEYRING: true\n");
    }
    std::fs::write(
        config_dir.join(bharatcode_core::config::base::CONFIG_YAML_NAME),
        contents,
    )
    .unwrap();
    config_dir
...
impl Provider for MockProvider {
    ...
    async fn stream(
        ...
        _session_id: &str,
        _system: &str,
        _messages: &[bharatcode_core::conversation::message::Message],
...
fn test_custom_get_extensions() {
    ...
    run_test(async move {
        ...
        let stored_inline_token = bharatcode_core::config::Config::global()
...
fn test_custom_preferences_read_save_remove() {
    let config_dir = write_acp_global_config(
        "BHARATCODE_MODEL: gpt-4o\nBHARATCODE_PROVIDER: openai\nBHARATCODE_AUTO_COMPACT_THRESHOLD: 0.7\nBHARATCODE_THINKING_EFFORT: high\nVOICE_AUTO_SUBMIT_PHRASES: send it\n",
    );
...
fn test_custom_defaults_read() {
    let config_dir = write_acp_global_config(
        "BHARATCODE_MODEL: claude-3-5-haiku-latest\nBHARATCODE_PROVIDER: anthropic\n",
    );
...
fn test_custom_dictation_secret_save_delete() {
    let _env = env_lock::lock_env([
        ("BHARATCODE_DISABLE_KEYRING", Some("1")),
