...
use bharatcode_core::permission::{Permission, PermissionConfirmation};
use bharatcode_core::providers::anthropic::ANTHROPIC_DEFAULT_MODEL;
use bharatcode_core::providers::azure::AZURE_DEFAULT_MODEL;
use bharatcode_core::providers::base::Provider;
#[cfg(feature = "aws-providers")]
use bharatcode_core::providers::bedrock::BEDROCK_DEFAULT_MODEL;
use bharatcode_core::providers::claude_code::CLAUDE_CODE_DEFAULT_MODEL;
use bharatcode_core::providers::codex::CODEX_DEFAULT_MODEL;
use bharatcode_core::providers::create_with_named_model;
use bharatcode_core::providers::databricks::DATABRICKS_DEFAULT_MODEL;
use bharatcode_core::providers::google::GOOGLE_DEFAULT_MODEL;
use bharatcode_core::providers::litellm::LITELLM_DEFAULT_MODEL;
use bharatcode_core::providers::openai::OPEN_AI_DEFAULT_MODEL;
#[cfg(feature = "aws-providers")]
use bharatcode_core::providers::sagemaker_tgi::SAGEMAKER_TGI_DEFAULT_MODEL;
use bharatcode_core::providers::snowflake::SNOWFLAKE_DEFAULT_MODEL;
use bharatcode_core::providers::xai::XAI_DEFAULT_MODEL;
...
impl ProviderFixture {
    async fn setup(config: &ProviderTestConfig, mode: GooseMode) -> Result<Self> {
        let mut env_vars: Vec<(&'static str, Option<&str>)> =
            vec![("BHARATCODE_MODE", Some(<&str>::from(mode)))];
    ...
    async fn tool_roundtrip(
        &self,
        prompt: &str,
        model_config: Option<bharatcode_providers::model::ModelConfig>,
    ) -> Result<Message> {
        let tools = self
            .agent
            .extension_manager
        ...
        let ctx = bharatcode_core::agents::ToolCallContext::new(
    ...
    async fn test_model_switch(&self) -> Result<()> {
        ...
        let alt_config =
            bharatcode_providers::model::ModelConfig::new(alt)?.with_canonical_limits(&self.name);
