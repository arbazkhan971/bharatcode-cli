...
use bharatcode_core::agents::extension_manager::{ExtensionManager, ExtensionManagerCapabilities};
...
async fn test_replayed_session(
    ...
    std::env::set_var("BHARATCODE_MCP_CLIENT_VERSION", "0.0.0");

    // Setup test file for developer extension tests
    let test_file_path = "/tmp/bharatcode_test/goose.txt";
    ...
    let mode = if env::var("BHARATCODE_RECORD_MCP").is_ok() {
        TestMode::Record
    } else {
    ...
    let session_manager = Arc::new(bharatcode_core::session::SessionManager::new(
