...
use bharatcode_core::providers::api_client::{ApiClient, AuthMethod as ApiAuthMethod};
...
pub async fn spawn_acp_server_in_process(
    ...
    let config_path = data_root.join(bharatcode_core::config::base::CONFIG_YAML_NAME);
    if !config_path.exists() {
        fs::write(
            &config_path,
            format!(
                "BHARATCODE_MODEL: {current_model}\nBHARATCODE_PROVIDER: openai\nBHARATCODE_MODE: {}\n",
                goose_mode
            ),
        )
        .unwrap();
    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    if std::env::var_os("BHARATCODE_PATH_ROOT").is_none() {
