...
pub struct LoggingConfig<'a> {
    ...
    /// Additional `EnvFilter` directives beyond the defaults (e.g. "bharatcode_server=info").
...
fn build_env_filter(extra_directives: &[&str]) -> EnvFilter {
    ...
    let mut filter = EnvFilter::new("")
        ...
        .add_directive("bharatcode=info".parse().unwrap())
...
///
/// This is the shared implementation used by both `bharatcode-cli` and `bharatcode-server`.
