...
impl Provider for AcpProvider {
    ...
    async fn stream(
        ...
        let goose_mode = *self
            .goose_mode
            .lock()
            .map_err(|_| ProviderError::RequestFailed("bharatcode_mode lock poisoned".into()))?;
...
fn build_handoff_context_memo(prior_messages: &[Message]) -> Option<String> {
    ...
    Some(format!(
        "Conversation context from bharatcode before this ACP provider session was created:\n\n\
{handoff_context}\n\n\
...
mod tests {
    ...
    fn get_model_config_surfaces_captured_context_size() {
        ...
        assert_eq!(
            ...
            bharatcode_providers::model::DEFAULT_CONTEXT_LIMIT
        );
    ...
    async fn failed_first_prompt_send_rolls_back_handoff_context_claim() {
        ...
        let result = provider
            .stream(&provider.model, "bharatcode-session", "", &messages, &[])
            .await;
    ...
    fn tool_meta_pairs_external_dispatch_marker_with_acp_kind() {
        ...
        for (kind, expected) in cases {
            ...
            assert_eq!(
                tool_meta["bharatcode.acp.kind"],
                serde_json::Value::String(expected.to_string()),
                "bharatcode.acp.kind serialized wrong for kind={kind:?}"
            );
        }
    }
}
