...
pub(super) const DEFAULT_PROVIDER_ID: &str = "bharatcode";
pub(super) const DEFAULT_PROVIDER_LABEL: &str = "BharatCode (Default)";
...
struct ClientCapabilitiesMeta {
    #[serde(rename = "bharatcode", default)]
...
fn extract_use_login_shell_path(args: &InitializeRequest) -> bool {
    args.meta
        .as_ref()
        .and_then(|meta| meta.get("bharatcode/useLoginShellPath"))
...
fn resolve_default_provider_model_config(
    config: &Config,
) -> Result<(String, bharatcode_providers::model::ModelConfig), agent_client_protocol::Error> {
    let resolved_provider = config.get_bharatcode_provider().map_err(|error| {
        agent_client_protocol::Error::internal_error()
            .data(format!("Failed to resolve provider: {}", error))
    })?;
    let resolved_model = config.get_bharatcode_model().map_err(|error| {
        agent_client_protocol::Error::internal_error()
...
async fn resolve_provider_default_model_config(
    provider_name: &str,
) -> Result<bharatcode_providers::model::ModelConfig, agent_client_protocol::Error> {
...
fn tool_call_identity_meta(tool_request: &ToolRequest) -> Option<Meta> {
    ...
    let extension_name = tool_request
        .tool_meta
        .as_ref()
        .and_then(|meta| meta.get("bharatcode_extension"))
...
impl GooseAcpAgent {
    ...
    async fn handle_tool_request(
        ...
        if Config::global()
            .get_bharatcode_disable_tool_call_summary()
            .unwrap_or(false)
        {
            return Ok(());
        }
...
fn message_update_meta(message_id: Option<&str>, created: i64, steer: bool) -> Meta {
    ...
    let mut meta = serde_json::Map::new();
    meta.insert("bharatcode".to_string(), serde_json::Value::Object(goose));
    meta
}
...
fn extract_tool_call_update_meta(
    ...
    meta_map.insert("bharatcode".to_string(), goose_meta);
    Some(meta_map)
}
...
impl GooseAcpAgent {
    async fn on_initialize(
        ...
        Ok(InitializeResponse::new(args.protocol_version)
            .agent_info(Implementation::new("bharatcode", env!("CARGO_PKG_VERSION")))
            .agent_capabilities(capabilities)
            .auth_methods(vec![AuthMethod::Agent(
                AuthMethodAgent::new("bharatcode-provider", "Configure Provider").description(
                    "Run `bharatcode configure` to set up your AI provider and API key",
                ),
            )]))
    }
    ...
    async fn on_set_thinking_effort(
        ...
        let effort = effort_id
            .parse::<bharatcode_providers::thinking::ThinkingEffort>()
            .map_err(|_| {
                agent_client_protocol::Error::invalid_params()
...
mod tests {
    ...
    use bharatcode_providers::conversation::token_usage::Usage as TokenUsage;
...
```",
    ...
    fn test_tool_call_identity_meta_uses_goose_extension_metadata() {
        let request = ToolRequest {
            ...
            metadata: None,
            tool_meta: Some(serde_json::json!({"bharatcode_extension": "context7"})),
        };
    ...
    fn with_tool_chain_summary_meta_preserves_existing_tool_call_identity() {
        ...
        let meta = with_tool_chain_summary_meta(existing, "ran two commands", 2)
            .expect("meta should be created");
        let goose = meta.get("bharatcode").expect("bharatcode key");
        assert_eq!(
            goose.get("toolCall"),
            Some(
    ...
    fn replay_attaches_chain_summary_meta_for_first_tool_request_with_persisted_summary() {
        ...
        let goose = meta
            ...
            .expect("replay meta must include a bharatcode namespace");
        assert_eq!(
            goose.get("toolCall"),
            Some(
                &serde_json::json!({ "toolName": "developer__shell", "extensionName": "developer" })
    ...
    fn test_merge_replay_message_meta_omits_steer_when_not_set() {
        ...
        assert_eq!(merged.get("bharatcode").and_then(|g| g.get("steer")), None);
    }
    ...
    fn test_build_usage_update_clamps_negative_used_to_zero() {
        ...
        session.model_config = Some(
            bharatcode_providers::model::ModelConfig::new("test-model")
                .unwrap()
