...
impl GooseAcpAgent {
    ...
    pub(super) async fn on_defaults_read(
        ...
        Ok(DefaultsReadResponse {
            provider_id: config.get_bharatcode_provider().ok(),
            model_id: config.get_bharatcode_model().ok(),
        })
    }
...
const PREFERENCE_DEFS: &[PreferenceDef] = &[
    PreferenceDef {
        ...
        config_key: "BHARATCODE_AUTO_COMPACT_THRESHOLD",
        prepare: prepare_auto_compact_threshold,
    },
    PreferenceDef {
        key: PreferenceKey::GooseThinkingEffort,
        config_key: "BHARATCODE_THINKING_EFFORT",
...
fn prepare_thinking_effort(
    ...
    let Some(value) = value.as_str() else {
        return Err(agent_client_protocol::Error::invalid_params()
            .data("bharatcodeThinkingEffort must be a string"));
    };
    let effort = value.parse::<ThinkingEffort>().map_err(|err| {
        agent_client_protocol::Error::invalid_params()
            .data(format!("Invalid bharatcodeThinkingEffort: {err}"))
    })?;
