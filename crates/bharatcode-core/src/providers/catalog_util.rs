pub use bharatcode_providers::canonical::catalog::{
...
pub async fn get_providers_by_format(format: ProviderFormat) -> Vec<ProviderCatalogEntry> {
    ...
    bharatcode_providers::canonical::catalog::get_providers_by_format(format, &native_provider_ids)
}
...
pub async fn get_setup_catalog_entries() -> Vec<ProviderSetupCatalogEntry> {
    ...
    bharatcode_providers::canonical::catalog::get_setup_catalog_entries(&registry_metadata)
}

pub fn get_provider_setup_category(provider_id: &str) -> Option<ProviderSetupCategory> {
    bharatcode_providers::canonical::catalog::get_provider_setup_category(provider_id)
}

pub fn get_provider_template(provider_id: &str) -> Option<ProviderTemplate> {
    bharatcode_providers::canonical::catalog::get_provider_template(provider_id)
}
...
mod tests {
    ...
    async fn setup_catalog_includes_goose_and_curated_fields() {
        ...
        let goose = entries
            .iter()
            .find(|entry| entry.provider_id == "bharatcode")
            .expect("setup catalog should include synthetic bharatcode");
