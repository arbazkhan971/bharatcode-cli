    ...
    fn databricks_v2_inventory_prefers_goose_model_ids_for_duplicate_names() {
        let models = enrich_model_ids_with_canonical(
            ...
            &[
                ...
                "bharatcode-gpt-5-5".to_string(),
            ],
        );

        assert!(
            models.iter().any(|model| model.id == "bharatcode-gpt-5-5"),
            "expected bharatcode-gpt-5-5 to win duplicate canonical-name tie, got {models:?}"
        );
        assert!(
            !models.iter().any(|model| model.id == "databricks-gpt-5-5"),
            "expected databricks-gpt-5-5 to be replaced by bharatcode-gpt-5-5, got {models:?}"
        );
    }
