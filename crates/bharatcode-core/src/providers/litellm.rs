...
impl Provider for LiteLLMProvider {
    ...
    async fn stream(
        ...
        let mut payload = bharatcode_providers::formats::openai::create_request(
            model_config,
            system,
            messages,
            tools,
        ...
        let message = bharatcode_providers::formats::openai::response_to_message(&response)?;
        let usage = bharatcode_providers::formats::openai::get_usage(&response);
