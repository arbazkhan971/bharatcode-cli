...
use bharatcode_providers::formats::openai::{create_request, get_usage, response_to_message};
...
impl ProviderDef for GithubCopilotProvider {
    ...
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            ...
            "GitHub Copilot",
            "GitHub Copilot. Run `bharatcode configure` and select copilot to set up.",
...
impl Provider for GithubCopilotProvider {
    ...
    async fn complete(
        ...
        // The prompt cache is wired centrally at the agent's streaming path
        // (see `reply_parts`), so this override only preserves the Copilot-specific
        // agent-call scoping that its `stream` implementation relies on.
        IS_AGENT_CALL
            .scope(true, async {
                collect_stream(
