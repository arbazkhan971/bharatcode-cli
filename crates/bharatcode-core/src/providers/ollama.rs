...
fn resolve_ollama_num_ctx(model_config: &ModelConfig) -> Option<usize> {
    ...
    let input_limit = match config.get_param::<usize>("BHARATCODE_INPUT_LIMIT") {
        Ok(limit) if limit > 0 => Some(limit),
        Ok(_) => None,
        Err(crate::config::ConfigError::NotFound(_)) => None,
        Err(e) => {
            tracing::warn!("Invalid BHARATCODE_INPUT_LIMIT value: {}", e);
            None
        }
    };
...
/// Configurable via OLLAMA_STREAM_TIMEOUT, BHARATCODE_STREAM_TIMEOUT, or falls back
...
/// Priority: OLLAMA_STREAM_TIMEOUT > BHARATCODE_STREAM_TIMEOUT > OLLAMA_TIMEOUT > default (120s).
...
fn resolve_ollama_chunk_timeout() -> u64 {
    ...
    if let Ok(val) = config.get_param::<u64>("BHARATCODE_STREAM_TIMEOUT") {
        if val > 0 {
            return val;
        }
    }
                ...
                Err(_) => {
                    Err::<(), anyhow::Error>(anyhow::anyhow!(
                        "Ollama stream stalled: no data received for {}s. \
                         ...
                         timeout via OLLAMA_STREAM_TIMEOUT, BHARATCODE_STREAM_TIMEOUT, or \
...
mod tests {
    ...
    fn test_apply_ollama_options_uses_input_limit() {
        let _guard = env_lock::lock_env([("BHARATCODE_INPUT_LIMIT", Some("8192"))]);
    ...
    fn test_apply_ollama_options_falls_back_to_context_limit() {
        let _guard = env_lock::lock_env([("BHARATCODE_INPUT_LIMIT", None::<&str>)]);
        let model_config = ModelConfig::new("qwen3")
            .unwrap()
    ...
    fn test_apply_ollama_options_preserves_stream_options_by_default() {
        ...
        let _guard = env_lock::lock_env([
            ("BHARATCODE_INPUT_LIMIT", None::<&str>),
    ...
    fn test_resolve_ollama_chunk_timeout_defaults_to_ollama_timeout() {
        let _guard = env_lock::lock_env([
            ...
            ("BHARATCODE_STREAM_TIMEOUT", None::<&str>),
    ...
    fn test_resolve_ollama_chunk_timeout_prefers_stream_override() {
        let _guard = env_lock::lock_env([
            ...
            ("BHARATCODE_STREAM_TIMEOUT", Some("90")),
            ("OLLAMA_TIMEOUT", Some("300")),
        ]);
    ...
    fn test_resolve_ollama_chunk_timeout_skips_zero_values() {
        let _guard = env_lock::lock_env([
            ...
            ("BHARATCODE_STREAM_TIMEOUT", Some("0")),
            ("OLLAMA_TIMEOUT", Some("300")),
        ]);
        assert_eq!(resolve_ollama_chunk_timeout(), 300);
    }
