...
impl OpenAiProvider {
    pub async fn from_env(
        ...
        // The data-residency / offline egress guard now runs centrally in the
        // shared API client (see `offline::install_egress_guard`), so every
        // request — for every provider — is screened in one place.
        ...
        // an explicit BHARATCODE_CONTEXT_LIMIT, an ACP/server per-session override, or a
        // BHARATCODE_PREDEFINED_MODELS entry, none of which we should overwrite. llama.cpp and
                ...
                match e {
                    ConfigError::NotFound(_) => {
                        if config.requires_auth {
                            anyhow::bail!(
                                "Required API key {} is not set. Configure it via `bharatcode configure` or set the {} environment variable.",
                                config.api_key_env,
                                config.api_key_env
                            );
                        }
