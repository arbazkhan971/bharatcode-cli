...
async fn poll_device_code_token(device: &DeviceCodeResponse) -> Result<TokenResponse> {
    ...
    loop {
        ...
        match body.error.as_deref() {
            ...
            Some("expired_token") => {
                return Err(anyhow!(
                    "xAI device code expired - please re-run bharatcode configure"
                ));
            }
            other => {
                let detail = body
...
<html>
  <head>
    <title>bharatcode - xAI Authorization Successful</title>
    <style>
      body {
...
<html>
  <head>
    <title>bharatcode - xAI Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
...
async fn perform_loopback_oauth_flow(auth_state: &XaiAuthState) -> Result<TokenData> {
    ...
    if webbrowser::open(&auth_url).is_err() {
        tracing::info!(
            "Please open this URL in your browser to authorize bharatcode with xAI:\n{}",
            auth_url
        );
    }
...
async fn perform_device_code_flow() -> Result<TokenData> {
    ...
    eprintln!(
        "\nTo authorize bharatcode with xAI, open this URL in any browser:\n  {}\nand enter code: {}\n",
...
mod tests {
    ...
    fn authorize_url_contains_required_oauth_params() {
        ...
        assert!(url.contains("referrer=bharatcode"));
