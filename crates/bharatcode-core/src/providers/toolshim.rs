...
pub const TOOLSHIM_BACKEND_ENV_VAR: &str = "BHARATCODE_TOOLSHIM_BACKEND";
pub const TOOLSHIM_LOCAL_MODEL_ENV_VAR: &str = "BHARATCODE_TOOLSHIM_MODEL";
...
/// - BHARATCODE_TOOLSHIM: When set to "true" or "1", enables using the tool shim in the standard OllamaProvider (default: false)
/// - BHARATCODE_TOOLSHIM_OLLAMA_MODEL: Ollama model to use as the tool interpreter (default: DEFAULT_INTERPRETER_MODEL)
...
";
        ...
        let interpreter_model = std::env::var("BHARATCODE_TOOLSHIM_OLLAMA_MODEL")
...
mod tests {
    ...
    fn parses_tokenized_tool_call_with_windows_path_arguments() {
        ...
        let content = "<|tool_calls_section_begin|> <|tool_call_begin|> functions.tree:0 <|tool_call_argument_begin|> {\"path\": \"C:\\Users\\eugen\\programmazione\\BharatCode-fork\", \"depth\": 1} <|tool_call_end|> <|tool_calls_section_end|>";
        let calls = parse_tokenized_tool_calls(content, &tools);
        ...
        assert_eq!(
            calls[0]
                ...
                .and_then(|v| v.as_str()),
            Some("C:\\Users\\eugen\\programmazione\\BharatCode-fork")
        );
    }
