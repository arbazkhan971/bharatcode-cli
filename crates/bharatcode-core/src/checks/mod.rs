...
///
/// Mirrors `bharatcode_core::agents::subagent_task_config::DEFAULT_SUBAGENT_MAX_TURNS`,
...
pub fn global_checks_dirs() -> Vec<PathBuf> {
    ...
    if let Some(home) = dirs_home() {
        dirs.push(home.join(".config").join("bharatcode").join("checks"));
    ...
    for entry in entries {
        ...
        match parsed {
            ...
            Err(e) => match mode {
                ...
                LoadMode::Lenient => {
                    eprintln!("bharatcode review: skipping {}: {e}", path.display());
                }
            },
        }
    }
