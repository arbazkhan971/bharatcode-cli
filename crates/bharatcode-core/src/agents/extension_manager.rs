...
fn resolve_timeout(timeout: Option<u64>) -> u64 {
    timeout.unwrap_or_else(|| {
        Config::global()
            .get_bharatcode_default_extension_timeout()
...
pub(crate) const TRUSTED_TOOL_UPDATE_META_KEY: &str = "__bharatcode_tool_update_meta";
...
const TOOL_EXTENSION_META_KEY: &str = "bharatcode_extension";
...
const BHARATCODE_USER_AGENT: reqwest::header::HeaderValue =
    reqwest::header::HeaderValue::from_static(concat!("bharatcode/", env!("CARGO_PKG_VERSION")));
...
async fn connect_with_auth(
    ...
    auth_headers.insert(reqwest::header::USER_AGENT, BHARATCODE_USER_AGENT);
...
async fn create_streamable_http_client(
    ...
    default_headers.insert(reqwest::header::USER_AGENT, BHARATCODE_USER_AGENT);
...
async fn create_unix_socket_http_client(
    ...
    custom_headers.insert(
        ...
        BHARATCODE_USER_AGENT
            .to_str()
            .unwrap_or("bharatcode")
            .parse()
            .unwrap_or_else(|_| HeaderValue::from_static("bharatcode")),
    );
...
impl ExtensionManager {
    ...
    pub async fn add_extension(
        ...
        let effective_working_dir = working_dir
            .clone()
            .or_else(|| {
                std::env::var("BHARATCODE_WORKING_DIR")
                    .ok()
                    .map(PathBuf::from)
            })
