...
use bharatcode_sdk_types::custom_requests::{SourceEntry, SourceType};
use rmcp::model::{
...
pub fn discover_filesystem_sources(working_dir: &Path) -> Vec<SourceEntry> {
    ...
    let local_recipe_dirs: Vec<PathBuf> = vec![
        working_dir.join(".bharatcode/recipes"),
        working_dir.join(".agents/recipes"),
    ];

    let global_recipe_dirs: Vec<PathBuf> = std::env::var("BHARATCODE_RECIPE_PATH")
        .ok()
        .into_iter()
        .flat_map(|p| {
        ...
        .chain(
            [
                home.as_ref().map(|h| h.join(".bharatcode/recipes")),
...
fn max_background_tasks() -> usize {
    Config::global()
        .get_param::<usize>("BHARATCODE_MAX_BACKGROUND_TASKS")
        .unwrap_or(5)
}

fn completed_task_ttl() -> Duration {
    let secs = Config::global()
        .get_param::<u64>("BHARATCODE_COMPLETED_TASK_TTL_SECS")
        .unwrap_or(600);
...
impl SummonClient {
        ...
        let schema = serde_json::json!({
            ...
            "properties": {
                ...
                "max_turns": {
                    "type": "integer",
                    "minimum": 1,
                    "description": "Maximum turns for this delegate. Overrides recipe settings.max_turns and BHARATCODE_SUBAGENT_MAX_TURNS."
                },
                "context": {
                    "type": "string",
        ...
        if sources.is_empty() && completed.is_empty() {
            return Ok(vec![Content::text(
                "No sources available for load/delegate.\n\n\
                 ...
                 • BHARATCODE_RECIPE_PATH directories",
            )]);
        }
    ...
    fn resolve_model_config(
        ...
        session: &crate::session::Session,
        provider_name: &str,
    ) -> Result<bharatcode_providers::model::ModelConfig, anyhow::Error> {
        ...
        let override_model = params
            ...
            .or_else(|| {
                Config::global()
                    .get_param::<String>("BHARATCODE_SUBAGENT_MODEL")
                    .ok()
            });
        ...
        if let Some(model) = override_model {
            if model != model_config.model_name {
                ...
                // overrides (BHARATCODE_CONTEXT_LIMIT, BHARATCODE_MAX_TOKENS) match the
    ...
    fn resolve_max_turns(&self, session: &crate::session::Session) -> usize {
        session
            ...
            .or_else(|| {
                std::env::var("BHARATCODE_SUBAGENT_MAX_TURNS")
                    .ok()
                    .and_then(|v| v.parse().ok())
            })
            .or_else(|| {
                Config::global()
                    .get_param::<usize>("BHARATCODE_SUBAGENT_MAX_TURNS")
                    .ok()
            })
...
You review code."#;
    ...
    async fn test_discover_recipes_and_agents() {
        ...
        let recipes = temp_dir.path().join(".bharatcode/recipes");
        ...
        let agents = temp_dir.path().join(".bharatcode/agents");
    ...
    async fn test_recipe_deduplication_local_wins() {
        ...
        let local = temp_dir.path().join(".bharatcode/recipes");
    ...
    fn test_resolve_max_turns_recipe_overrides_env_var() {
        ...
        std::env::set_var("BHARATCODE_SUBAGENT_MAX_TURNS", "99");
        let result = client.resolve_max_turns(&session);
        std::env::remove_var("BHARATCODE_SUBAGENT_MAX_TURNS");
    ...
    fn test_resolve_max_turns_falls_back_to_env_var() {
        ...
        std::env::set_var("BHARATCODE_SUBAGENT_MAX_TURNS", "7");
        let result = client.resolve_max_turns(&session);
        std::env::remove_var("BHARATCODE_SUBAGENT_MAX_TURNS");

        assert_eq!(
            result, 7,
            "should fall back to BHARATCODE_SUBAGENT_MAX_TURNS env var"
        );
    }
    ...
    fn session_with(parent: bharatcode_providers::model::ModelConfig) -> crate::session::Session {
    ...
    fn resolve_with_override(
        model: Option<&str>,
        parent: bharatcode_providers::model::ModelConfig,
    ) -> bharatcode_providers::model::ModelConfig {
        let client = SummonClient::new(create_test_context()).unwrap();
        let params = DelegateParams {
    ...
    fn parent_config() -> bharatcode_providers::model::ModelConfig {
        bharatcode_providers::model::ModelConfig::new(PARENT_MODEL)
            .unwrap()
    ...
    async fn test_resolve_model_config_applies_canonical_limits_to_overridden_model() {
        let _env = env_lock::lock_env([
            ...
            ("BHARATCODE_SUBAGENT_MODEL", None::<&str>),
        ]);

        let parent = parent_config();
        let overridden = bharatcode_providers::model::ModelConfig::new(OVERRIDE_MODEL)
            .unwrap()
            .with_canonical_limits(PROVIDER);
