...
impl fmt::Display for GoosePlatform {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            GoosePlatform::GooseCli => write!(f, "bharatcode-cli"),
            GoosePlatform::GooseDesktop => write!(f, "bharatcode-desktop"),
        }
    }
}
...
impl Agent {
    pub fn new() -> Self {
        ...
        Self::with_config(AgentConfig::new(
            ...
            None,
            config.get_bharatcode_mode().unwrap_or_default(),
            config
                .get_bharatcode_disable_session_naming()
                .unwrap_or(false),
    ...
    fn stop_hook_block_cap(&self) -> u32 {
        ...
        Config::global()
            .get_param::<u32>("BHARATCODE_STOP_HOOK_BLOCK_CAP")
    ...
    fn create_tool_inspection_manager(
        ...
        // Add adversary inspector (LLM-based review, enabled by ~/.config/bharatcode/adversary.md)
    ...
    async fn prepare_reply_context(
        ...
        let tool_call_cut_off = match Config::global()
            .get_param::<usize>("BHARATCODE_TOOL_CALL_CUTOFF")
        {
            Ok(v) => v,
            Err(_) => {
                let context_limit = self
                    ...
                    .unwrap_or(bharatcode_providers::model::DEFAULT_CONTEXT_LIMIT);
                let compaction_threshold = Config::global()
                    .get_param::<f64>("BHARATCODE_AUTO_COMPACT_THRESHOLD")
    ...
    async fn reply_internal(
        ...
        let reply_stream_span = tracing::info_span!(
            target: "bharatcode::agents::agent",
            "reply_stream",
        ...
        let inner = Box::pin(async_stream::try_stream! {
            ...
            let max_turns = session_config.max_turns.unwrap_or_else(|| {
                Config::global()
                    .get_param::<u32>("BHARATCODE_MAX_TURNS")
            ...
            if !is_token_cancelled(&cancel_token) {
                if let Some(status_line) = verify::verify_and_format(&working_dir).await {
                    yield AgentEvent::Message(
                        Message::assistant().with_system_notification(
                            SystemNotificationType::InlineMessage,
                            status_line,
    ...
    pub async fn update_goose_mode(&self, mode: GooseMode, session_id: &str) -> Result<()> {
        ...
        *self.current_goose_mode.lock().await = mode;
        self.config
            ...
            .apply()
            .await
            .context("Failed to persist bharatcode_mode to session")
    }
...
mod tests {
    ...
    impl crate::providers::base::Provider for ActionRequiredProvider {
        ...
        fn get_model_config(&self) -> bharatcode_providers::model::ModelConfig {
            bharatcode_providers::model::ModelConfig::new("test").unwrap()
        }
        async fn stream(
            &self,
            _: &bharatcode_providers::model::ModelConfig,
            _: &str,
            _: &str,
...
"#,
    ...
    impl crate::providers::base::Provider for CountingTextProvider {
        async fn stream(
            &self,
            _model_config: &bharatcode_providers::model::ModelConfig,
            _session_id: &str,
            _system_prompt: &str,
            _messages: &[Message],
            _tools: &[Tool],
        ...
        fn get_model_config(&self) -> bharatcode_providers::model::ModelConfig {
            bharatcode_providers::model::ModelConfig::new("mock-model").unwrap()
        }
                ...
                matches!(
                    ...
                    MessageContent::SystemNotification(notification)
                        if notification.msg.contains("more than 2 consecutive times")
                            && notification.msg.contains("BHARATCODE_STOP_HOOK_BLOCK_CAP")
                )
            })
        }));
    ...
    async fn test_add_final_output_tool() -> Result<()> {
        ...
        let system_prompt = prompt_manager
            .builder()
            .with_bharatcode_mode(GooseMode::default())
            .build();
