...
fn is_global_agent_file(path: &Path) -> bool {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        global_roots.push(home.join(".bharatcode").join("agents"));
...
fn resolve_agent_file_with_roots(
    ...
    let in_agent_dir = parent_name == Some("agents")
        && matches!(
            grandparent_name,
            Some(".bharatcode") | Some(".claude") | Some(".agents")
        );
...
fn list_agent_dirs(working_dir: Option<&Path>, additional_roots: &[SourceRoot]) -> Vec<SourceRoot> {
    ...
    if let Some(working_dir) = working_dir {
        ...
        dirs.push(SourceRoot {
            path: working_dir.join(".bharatcode").join("agents"),
            writable: true,
        });
        dirs.push(SourceRoot {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        dirs.push(SourceRoot {
            path: home.join(".bharatcode").join("agents"),
            writable: true,
        });
        dirs.push(SourceRoot {
...
mod tests {
    ...
    fn list_sources_lists_builtin_skills() {
        ...
        let builtin = listed
            .iter()
            .find(|source| source.name == "bharatcode-doc-guide")
        ...
        assert_eq!(builtin.path, "builtin://skills/bharatcode-doc-guide");
    ...
    fn filesystem_skill_suppresses_same_named_builtin() {
        ...
        let skill_dir = project
            .join(".agents")
            .join("skills")
            .join("bharatcode-doc-guide");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
            skill_dir.join("SKILL.md"),
            build_skill_md(
                "bharatcode-doc-guide",
                "project override",
                "Use project docs",
                &HashMap::new(),
            ),
        ...
        assert!(!builtins
            .iter()
            .any(|source| source.name == "bharatcode-doc-guide"));
    ...
    fn project_sources_prefer_agents_directory_over_legacy_goose() {
        ...
        std::fs::write(
            ...
            build_skill_md("shared-skill", "legacy", "BharatCode", &HashMap::new()),
        )
        .unwrap();
    ...
    fn update_rejects_path_traversal() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let escaped_dir = project.join(".bharatcode").join("escaped");
        ...
        let attempted_escape = project.join(".bharatcode").join("escaped");
        let err = update_source_with_roots(
            SourceType::Skill,
