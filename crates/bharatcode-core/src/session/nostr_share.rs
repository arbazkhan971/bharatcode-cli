...
pub const CONFIG_RELAYS_KEY: &str = "BHARATCODE_NOSTR_RELAYS";
    ...
    P: NostrPublisher + Sync,
{
    ...
    let event = EventBuilder::new(Kind::Custom(EVENT_KIND), encrypted)
        ...
        .tag(Tag::parse(["client", "bharatcode"])?)
...
pub fn build_deeplink(nevent: &str, decryption_key: &str) -> String {
    format!(
        "bharatcode://sessions/nostr?nevent={}&key={}",
...
pub fn parse_deeplink(deeplink: &str) -> Result<ParsedShareLink> {
    let parsed = url::Url::parse(deeplink).context("Invalid BharatCode session share link")?;
    if parsed.scheme() != "bharatcode"
        || parsed.host_str() != Some("sessions")
        || parsed.path() != "/nostr"
    {
        return Err(anyhow!("Invalid BharatCode Nostr session share link"));
    }
...
mod tests {
    ...
    async fn publish_builds_deeplink_and_encrypted_kind_30278_event() {
        ...
        assert!(share.deeplink.starts_with("bharatcode://sessions/nostr?"));
    ...
    #[test]
    fn parses_deeplink() {
        let parsed = parse_deeplink("bharatcode://sessions/nostr?nevent=abc&key=def").unwrap();
