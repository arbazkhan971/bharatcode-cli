...
pub const BHARATCODE_HINTS_FILENAME: &str = ".bharatcodehints";
...
pub fn load_hint_files(
    ...
    if !global_hints_contents.is_empty() {
        hints.push_str("\n### Global Hints\nThese are my global bharatcode hints.\n");
...
mod tests {
    ...
    fn test_goosehints_when_present() {
        ...
        fs::write(
            dir.path().join(BHARATCODE_HINTS_FILENAME),
            "Test hint content",
        )
        .unwrap();
    ...
    fn test_goosehints_multiple_filenames() {
        ...
        fs::write(
            dir.path().join(BHARATCODE_HINTS_FILENAME),
            "Custom hints file content from .bharatcodehints",
        )
        .unwrap();
        ...
        assert!(hints.contains("Custom hints file content from .bharatcodehints"));
    }
    ...
    fn test_goosehints_configurable_filename() {
        ...
        assert!(!hints.contains(".bharatcodehints")); // Make sure it's not loading the default
    }
    ...
    fn test_nested_goosehints_with_git_root() {
        ...
        fs::write(
            project_root.join(BHARATCODE_HINTS_FILENAME),
            "Root hints content",
        )
        .unwrap();
        ...
        fs::write(
            subdir.join(BHARATCODE_HINTS_FILENAME),
        ...
        fs::write(
            current_dir.join(BHARATCODE_HINTS_FILENAME),
    ...
    fn test_nested_goosehints_without_git_root() {
        ...
        fs::write(
            base_dir.join(BHARATCODE_HINTS_FILENAME),
            "Base hints content",
        )
        .unwrap();
    ...
    fn test_nested_goosehints_mixed_filenames() {
        ...
        fs::write(
            subdir.join(BHARATCODE_HINTS_FILENAME),
            "Subdir .bharatcodehints content",
        )
        .unwrap();
        ...
        assert!(hints.contains("Subdir .bharatcodehints content"));
    }
...
Additional instructions here."#;
        fs::write(project_root.join(BHARATCODE_HINTS_FILENAME), hints_content).unwrap();
...
End of nested hints"#;
        fs::write(
            components_dir.join(BHARATCODE_HINTS_FILENAME),
...
@../parent.md
End of hints"#;
        fs::write(current_dir.join(BHARATCODE_HINTS_FILENAME), hints_content).unwrap();
...
@../root_file.md
End of hints"#;
        fs::write(subdir.join(BHARATCODE_HINTS_FILENAME), hints_content).unwrap();
        let gitignore = create_dummy_gitignore();
    ...
    fn tracker_deduplicates_directories() {
        ...
        let subdir = project_root.join("nested");
        fs::create_dir_all(&subdir).unwrap();
        fs::write(subdir.join(BHARATCODE_HINTS_FILENAME), "nested hints").unwrap();
