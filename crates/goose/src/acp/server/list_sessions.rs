...
fn include_last_message_snippet_from_meta(
    ...
    let Some(value) = meta.and_then(|meta| meta.get("bharatcode")) else {
        return Ok(false);
    };
    if value.is_null() {
        return Ok(false);
    ...
    let Some(goose_meta) = value.as_object() else {
        return Err(
            agent_client_protocol::Error::invalid_params().data("bharatcode must be an object")
        );
    };
    ...
    value.as_bool().ok_or_else(|| {
        agent_client_protocol::Error::invalid_params()
            .data("bharatcode.includeLastMessageSnippet must be a boolean")
    })
}
