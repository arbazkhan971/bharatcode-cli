...
const BHARATCODE_CONFIG_PREFIX: &str = "bharatcode_config:";
...
fn import_failure_warning(
    ...
    let source_name = match source_kind {
        OnboardingImportSourceKind::GooseConfig => "BharatCode configuration",
...
fn scan_goose_config_candidate(path: &Path) -> Option<OnboardingImportCandidate> {
    ...
    if mapping_contains_string(&mapping, "BHARATCODE_PROVIDER")
        || mapping_contains_string(&mapping, "BHARATCODE_MODEL")
    {
        counts.providers = 1;
    }
    ...
    if counts.sessions > 0 {
        warnings.push("Sessions are already shared through BharatCode's data store.".to_string());
    }

    Some(OnboardingImportCandidate {
        id: candidate_id(BHARATCODE_CONFIG_PREFIX, path),
        source_kind: OnboardingImportSourceKind::GooseConfig,
        display_name: "Existing BharatCode configuration".to_string(),
...
fn parse_candidate_id(id: &str) -> Option<(OnboardingImportSourceKind, PathBuf)> {
    if let Some(path) = id.strip_prefix(BHARATCODE_CONFIG_PREFIX) {
...
fn apply_goose_config_candidate(
    ...
    let provider = yaml_string(&source, "BHARATCODE_PROVIDER");
    let model = yaml_string(&source, "BHARATCODE_MODEL");
    ...
    result.warnings.push(
        "Session history already lives in the BharatCode data store when available.".to_string(),
    );
    Ok(result)
}
...
mod tests {
    ...
    fn apply_onboarding_imports_continues_after_candidate_failure() {
        ...
        let req = OnboardingImportApplyRequest {
            candidate_ids: vec![
                candidate_id(BHARATCODE_CONFIG_PREFIX, &missing_goose_config),
        ...
        assert!(response
            .warnings
            .iter()
            .any(|warning| warning.starts_with("Skipped BharatCode configuration import at ")));
...
"#,
        ...
        assert_eq!(target_config.get_bharatcode_provider().unwrap(), "openai");
    ...
    fn apply_goose_config_model_only_skips_provider_activation() {
        ...
        let target = TempDir::new().unwrap();
        let source_config = source.path().join(CONFIG_YAML_NAME);
        fs::write(&source_config, "BHARATCODE_MODEL: gpt-5.1\n").unwrap();
        ...
        // Local-first resolution always yields a default provider, so verify the
        // import skipped activation by asserting no active provider was persisted.
        assert!(target_config
            .get_param::<String>("active_provider")
            .is_err());
    }
