...
const INSTALL_METADATA: &str = ".bharatcode-plugin-install.json";
...
mod tests {
    ...
    fn init_git_repo(repo: &Path) {
        run_git(repo, &["init"]);
        run_git(repo, &["config", "user.email", "bharatcode@example.com"]);
        run_git(repo, &["config", "user.name", "BharatCode"]);
    }
