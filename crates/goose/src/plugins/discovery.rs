...
fn user_settings_path() -> Option<PathBuf> {
    if let Ok(test_root) = std::env::var("BHARATCODE_PATH_ROOT") {
        return Some(
...
fn project_settings_path(project_root: &Path, local: bool) -> PathBuf {
    ...
    project_root.join(".config").join("bharatcode").join(file)
}
...
mod tests {
    ...
    fn test_config(dir: &Path) -> Config {
        Config::new(dir.join("config.yaml"), "bharatcode-discovery-test").unwrap()
    }
    ...
    fn disabled_in_project_settings_drops_plugin() {
        ...
        write_settings(
            &project.join(".config").join("bharatcode"),
    ...
    fn project_scope_overrides_user_scope() {
        ...
        write_settings(
            &fake_home.path().join(".config").join("bharatcode"),
            r#"{"disabledPlugins":["demo"]}"#,
        );
