...
impl<'a> SystemPromptBuilder<'a, PromptManager> {
    ...
    pub fn with_bharatcode_mode(mut self, mode: GooseMode) -> Self {
    ...
    pub fn build(self) -> String {
        ...
        let goose_mode = self
            .goose_mode
            .unwrap_or_else(|| Config::global().get_bharatcode_mode().unwrap_or_default());
        ...
        .unwrap_or_else(|_| {
            "You are a general-purpose AI agent called BharatCode, an open-source, India-first coding agent".to_string()
        });
...
mod tests {
    ...
    async fn test_all_platform_extensions() {
        ...
        let _guard = env_lock::lock_env([
            ...
            ("BHARATCODE_PATH_ROOT", Some(temp_root.as_str())),
        ]);
