...
/// Resolve the preferred Unix shell for command execution, respecting BHARATCODE_SHELL.
///
...
/// the host filesystem. `BHARATCODE_SHELL` is passed through as-is.
///
#[cfg(windows)]
fn windows_shell() -> String {
    std::env::var("BHARATCODE_SHELL").unwrap_or_else(|_| "cmd".to_string())
}
...
fn unix_shell() -> String {
    if let Ok(shell) = std::env::var("BHARATCODE_SHELL") {
        return shell;
    }
...
/// Opt-in exec sandbox level, read from `BHARATCODE_SANDBOX`. Defaults to
/// [`SandboxMode::Off`] so the shell tool, the portable-default build, and CI
/// containers without landlock/seccomp support behave exactly as before.
#[cfg(not(windows))]
...
/// Resolve the sandbox level from `BHARATCODE_SANDBOX`
/// (`off` | `read-only` | `workspace-write`, default `off`).
#[cfg(not(windows))]
fn sandbox_mode() -> SandboxMode {
    match std::env::var("BHARATCODE_SANDBOX").ok().as_deref() {
        Some("read-only") | Some("readonly") | Some("read_only") => SandboxMode::ReadOnly,
        Some("workspace-write") | Some("workspace_write") => SandboxMode::WorkspaceWrite,
        _ => SandboxMode::Off,
    }
}
...
impl ShellTool {
    ...
    pub async fn shell_with_cwd(
        ...
        // Opt-in exec policy gate (default off). Screens the command against
        // command-prefix allow/deny lists before anything is spawned.
        if let crate::exec_policy::Decision::Deny { reason } =
            crate::exec_policy::check_command(&params.command)
        {
            return Self::error_result(&reason, None);
        }
    ...
    let mut command = {
        ...
        if is_flatpak() {
            ...
            if mode != SandboxMode::Off {
                let writable_roots = match (mode, working_dir) {
                    (SandboxMode::WorkspaceWrite, Some(dir)) => vec![dir.to_path_buf()],
                    _ => Vec::new(),
                };
                let policy = bharatcode_linux_sandbox::SandboxPolicy {
                    writable_roots,
                    allow_network: mode == SandboxMode::WorkspaceWrite,
                };
