...
///
/// The real (un-zeroed) usage is stored — cost is only zeroed when the entry is
/// later served as a hit. Errors are propagated and never cached.
fn store_completion_on_complete(inner: MessageStream, key: String) -> MessageStream {
    Box::pin(try_stream! {
        let mut inner = inner;
        let mut acc_message: Option<Message> = None;
        let mut acc_usage: Option<ProviderUsage> = None;

        while let Some(result) = inner.next().await {
            let (msg_opt, usage_opt) = result?;

            if let Some(msg) = &msg_opt {
                acc_message = Some(match acc_message.take() {
                    Some(mut prev) => {
                        for new_content in msg.content.clone() {
            ...
            if let Some(usage) = &usage_opt {
                acc_usage = Some(usage.clone());
            }

            yield (msg_opt, usage_opt);
        }

        if let Some(message) = acc_message {
            let usage = acc_usage
                .unwrap_or_else(|| ProviderUsage::new("unknown".to_string(), Usage::default()));
            crate::prompt_cache::store(
                &key,
                &crate::prompt_cache::CachedCompletion { message, usage },
            );
        }
    })
}
...
impl Agent {
    pub async fn prepare_tools_and_prompt(
        ...
        let mut system_prompt = prompt_manager
            ...
            .with_bharatcode_mode(goose_mode)
            .build();
    ...
    pub(crate) async fn stream_response_from_provider(
        ...
        // Opt-in prompt/response cache (BHARATCODE_CACHE). The agent loop streams
        // responses via this function, so the cache is wired here — the streaming
        // path the agent actually uses — rather than only on the non-streaming
        // `complete()` path. No-op when the cache is disabled.
        let cache_key = crate::prompt_cache::is_enabled().then(|| {
            crate::prompt_cache::cache_key(
                provider.get_name(),
        ...
        if let Some(key) = &cache_key {
            if let Some(hit) = crate::prompt_cache::lookup(key) {
                debug!("prompt cache hit (stream path)");
                return Ok(stream_from_single_message(
                    hit.message,
                    crate::prompt_cache::zero_cost_usage(&hit.usage),
                ));
            }
        }
        ...
        // On a cache miss with caching enabled, tee the stream so the fully
        // assembled response is written to the cache once it completes.
        match cache_key {
            Some(key) => Ok(store_completion_on_complete(produced, key)),
            None => Ok(produced),
        }
    }
...
mod tests {
    ...
    async fn categorize_tool_requests_skips_externally_dispatched_and_preserves_marker() {
        ...
        assert!(
            ...
            "bharatcode.external_dispatch marker was clobbered by coercion; merged tool_meta = {:?}",
            tool_req.tool_meta
        );
        let merged = tool_req
            .tool_meta
