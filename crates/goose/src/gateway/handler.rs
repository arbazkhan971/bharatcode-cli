...
/// keeps a stricter default than the global `BHARATCODE_MAX_TURNS` ceiling.  Users
/// can override this through `BHARATCODE_GATEWAY_MAX_TURNS` (gateway-specific) or
/// `BHARATCODE_MAX_TURNS` (applies globally).
const DEFAULT_GATEWAY_MAX_TURNS: u32 = 5;

/// Resolve the max turns to use for a gateway session.
///
/// Precedence: `BHARATCODE_GATEWAY_MAX_TURNS` -> `BHARATCODE_MAX_TURNS` ->
...
impl GatewayHandler {
                ...
                if let Some(gateway_type) = self.try_consume_code(message.text.trim()).await? {
                    ...
                    self.gateway
                        .send_message(
                            &message.user,
                            OutgoingMessage::Text {
                                body: "Welcome! Enter your pairing code to connect to bharatcode."
                                    .into(),
                            },
                        )
                        .await?;
    ...
    async fn complete_pairing(&self, user: &PlatformUser) -> anyhow::Result<()> {
        ...
        let provider = config.get_bharatcode_provider().ok();
        if let Some(ref provider) = provider {
            update = update.provider_name(provider);
        }
        if let (Some(ref provider), Ok(model_name)) = (&provider, config.get_bharatcode_model()) {
        ...
        self.gateway
            .send_message(
                user,
                OutgoingMessage::Text {
                    body: "Paired! You can now chat with bharatcode.".into(),
                },
            )
            .await?;
    ...
    async fn sync_session_config(&self, session: &Session) -> anyhow::Result<bool> {
        ...
        let current_provider = config.get_bharatcode_provider().ok();
        let current_model_name = config.get_bharatcode_model().ok();
        ...
        let current_mode = config.get_bharatcode_mode().unwrap_or_default();
    ...
    async fn relay_to_session(
        ...
        // whatever it has.
        //
        // Honors `BHARATCODE_GATEWAY_MAX_TURNS` (gateway-specific override) and
        // `BHARATCODE_MAX_TURNS` (global), falling back to a conservative default
        // so the limit is configurable without editing the source.
        let config = Config::global();
        let max_turns = resolve_gateway_max_turns(
            config.get_param::<u32>("BHARATCODE_GATEWAY_MAX_TURNS").ok(),
            config.get_param::<u32>("BHARATCODE_MAX_TURNS").ok(),
        );
