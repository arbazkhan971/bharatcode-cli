...
impl TelegramGateway {
    ...
    fn save_voice_file(
        ...
        let dir = std::env::temp_dir().join("bharatcode_voice");
...
mod tests {
    ...
    fn voice_prompt_includes_path_and_duration() {
        let path = std::path::PathBuf::from("/tmp/bharatcode_voice/voice_test.ogg");
        let prompt = TelegramGateway::voice_prompt(&path, Some(10), Some("audio/ogg"));
        assert!(prompt.contains("/tmp/bharatcode_voice/voice_test.ogg"));
    ...
    fn voice_prompt_with_mp3_mime() {
        let path = std::path::PathBuf::from("/tmp/bharatcode_voice/voice_test.mp3");
