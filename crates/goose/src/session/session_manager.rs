...
impl sqlx::FromRow<'_, sqlx::sqlite::SqliteRow> for Session {
    fn from_row(row: &sqlx::sqlite::SqliteRow) -> Result<Self, sqlx::Error> {
        ...
        Ok(Session {
            ...
            model_config,
            goose_mode: row
                .try_get::<String, _>("bharatcode_mode")
                .ok()
...
impl SessionStorage {
    ...
    async fn create_schema(pool: &Pool<Sqlite>) -> Result<()> {
        ...
        sqlx::query(
            ...
            CREATE TABLE IF NOT EXISTS sessions (
                ...
                bharatcode_mode TEXT NOT NULL DEFAULT 'auto',
    ...
    async fn import_legacy_session(pool: &Pool<Sqlite>, session: &Session) -> Result<()> {
        ...
        INSERT INTO sessions (
            ...
            provider_name, model_config_json, bharatcode_mode
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        "#,
        )
        .bind(&session.id)
    ...
    async fn apply_migration(tx: &mut sqlx::Transaction<'_, Sqlite>, version: i32) -> Result<()> {
        match version {
            ...
            8 => {
                sqlx::query(
                    r#"
                    ALTER TABLE sessions ADD COLUMN bharatcode_mode TEXT NOT NULL DEFAULT 'auto'
                "#,
                )
                .execute(&mut **tx)
                .await?;
            ...
            15 => {
                let has_bharatcode_mode = sqlx::query_scalar::<_, i32>(
                    "SELECT COUNT(*) FROM pragma_table_info('sessions') WHERE name = 'bharatcode_mode'",
                )
                .fetch_one(&mut **tx)
                .await?
                    > 0;
                if !has_bharatcode_mode {
                    ...
                    if has_goose_mode {
                        sqlx::query(
                            "ALTER TABLE sessions RENAME COLUMN goose_mode TO bharatcode_mode",
                        )
                        .execute(&mut **tx)
                        .await?;
                    } else {
    ...
    async fn create_session(
        ...
        let session = sqlx::query_as(
            r#"
                INSERT INTO sessions (id, name, user_set_name, session_type, working_dir, extension_data, bharatcode_mode)
                VALUES (
                    ? || '_' || CAST(COALESCE((
    ...
    async fn apply_update(&self, builder: SessionUpdateBuilder<'_>) -> Result<()> {
        ...
        add_update!(builder.goose_mode, "bharatcode_mode");
    ...
    async fn list_sessions_matching(&self, query: SessionListQuery<'_>) -> Result<Vec<Session>> {
        ...
        let sql = format!(
            ...
            SELECT s.id, s.working_dir, s.name, s.description, s.user_set_name, s.session_type, s.created_at, s.updated_at, s.extension_data,
                   ...
                   s.provider_name, s.model_config_json, s.bharatcode_mode,
...
mod tests {
    ...
    async fn test_goose_mode_malformed_defaults_to_auto() {
        ...
        sqlx::query("UPDATE sessions SET bharatcode_mode = 'garbage' WHERE id = ?")
            .bind(&session.id)
            .execute(pool)
            .await
            .unwrap();
