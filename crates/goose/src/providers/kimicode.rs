...
impl ProviderDef for KimiCodeProvider {
    ...
    fn metadata() -> ProviderMetadata {
        ...
        .with_setup_steps(vec![
            "Run `bharatcode configure` and select 'Kimi Code'",
            "A browser window will open — log in to kimi.com and enter the displayed code",
            "Once authorized, BharatCode will save your token automatically",
        ])
    }
        ...
        KimiCodeProvider {
            ...
            token_cache: TokenCache {
                path: std::env::temp_dir()
                    .join(format!("bharatcode-kimicode-test-{}.json", Uuid::new_v4())),
            },
            cached_token: tokio::sync::Mutex::new(None),
