...
/// Provider used when nothing is configured, making fresh installs local-first.
/// Cloud providers stay available via env vars or explicit `active_provider`.
const DEFAULT_LOCAL_PROVIDER: &str = crate::providers::ollama::OLLAMA_PROVIDER_NAME;
...
/// providers destructively overwrote `BHARATCODE_PROVIDER` / `BHARATCODE_MODEL`.
...
///
/// Resolution order:
/// 1. `BHARATCODE_PROVIDER` environment variable (uppercase check performed by
///    `get_param`)
/// 2. `active_provider` key in config.yaml
/// 3. Legacy flat `BHARATCODE_PROVIDER` key in config.yaml (backward compat)
/// 4. Local-first default ([`DEFAULT_LOCAL_PROVIDER`]) when nothing is configured
pub fn get_active_provider(config: &Config) -> Option<String> {
    // Env var takes precedence (get_param checks env automatically)
    if let Ok(val) = env::var("BHARATCODE_PROVIDER") {
        return Some(val);
    }
    ...
    // Legacy flat key fallback, then local-first default
    config
        .get_param::<String>("BHARATCODE_PROVIDER")
        .ok()
        .or_else(|| Some(DEFAULT_LOCAL_PROVIDER.to_string()))
}

/// Return the model for the currently active provider.
///
/// Resolution order:
/// 1. `BHARATCODE_MODEL` environment variable
/// 2. Model recorded in the active provider's entry (`providers.{name}.model`)
/// 3. Legacy flat `BHARATCODE_MODEL` key in config.yaml
pub fn get_active_model(config: &Config) -> Option<String> {
    // Env var takes precedence
    if let Ok(val) = env::var("BHARATCODE_MODEL") {
        return Some(val);
    }
    ...
    // Legacy flat key fallback, then the local default's model when local-first
    config
        .get_param::<String>("BHARATCODE_MODEL")
        .ok()
        .or_else(|| {
            (get_active_provider(config).as_deref() == Some(DEFAULT_LOCAL_PROVIDER))
                .then(|| crate::providers::ollama::OLLAMA_DEFAULT_MODEL.to_string())
        })
}
...
mod tests {
    ...
    fn test_get_active_provider_falls_back_to_legacy() {
        let config = new_test_config();
        config
            .set_param("BHARATCODE_PROVIDER", "anthropic")
            .unwrap();
    ...
    #[test]
    fn test_get_active_provider_defaults_to_local() {
        let config = new_test_config();
        let result = get_active_provider(&config);
        assert_eq!(result, Some(DEFAULT_LOCAL_PROVIDER.to_string()));
    }

    #[test]
    fn test_get_active_model_defaults_to_local() {
        let config = new_test_config();
        let result = get_active_model(&config);
        assert_eq!(
            result,
            Some(crate::providers::ollama::OLLAMA_DEFAULT_MODEL.to_string())
        );
    }
    ...
    fn test_get_active_model_falls_back_to_legacy() {
        ...
        config
            .set_param("BHARATCODE_MODEL", "gpt-3.5-turbo")
            .unwrap();
