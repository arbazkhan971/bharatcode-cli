...
mod tests {
    ...
    #[test]
    fn test_sarvam_json_deserializes() {
        let json = include_str!("../providers/declarative/sarvam.json");
        let config: DeclarativeProviderConfig =
            serde_json::from_str(json).expect("sarvam.json should parse");
        assert_eq!(config.name, "sarvam");
        assert_eq!(config.display_name, "Sarvam AI");
        assert!(matches!(config.engine, ProviderEngine::OpenAI));
        assert_eq!(config.api_key_env, "SARVAM_API_KEY");
        assert_eq!(config.base_url, "https://api.sarvam.ai/v1");
        assert_eq!(config.dynamic_models, Some(false));
        assert_eq!(config.models[0].name, "sarvam-m");
    }

    #[test]
    fn test_krutrim_json_deserializes() {
        let json = include_str!("../providers/declarative/krutrim.json");
        let config: DeclarativeProviderConfig =
            serde_json::from_str(json).expect("krutrim.json should parse");
        assert_eq!(config.name, "krutrim");
        assert_eq!(config.display_name, "Krutrim (Ola)");
        assert!(matches!(config.engine, ProviderEngine::OpenAI));
        assert_eq!(config.api_key_env, "KRUTRIM_API_KEY");
        assert_eq!(config.base_url, "https://cloud.krutrim.com/v1");
        assert_eq!(config.dynamic_models, Some(false));
        assert_eq!(config.models[0].name, "Llama-3.3-70B-Instruct");
    }

    #[test]
    fn test_india_presets_register_in_fixed_providers() {
        let configs = load_fixed_providers().expect("fixed providers should load");
        for expected in ["sarvam", "krutrim"] {
            assert!(
                configs.iter().any(|c| c.name == expected),
                "declarative provider '{expected}' missing from FIXED_PROVIDERS"
            );
        }
    }
