...
pub fn provider_tls_config_from_config(config: &Config) -> Result<Option<TlsConfig>> {
    ...
    let client_cert_path = config
        .get_param::<String>("BHARATCODE_CLIENT_CERT_PATH")
        .ok();
    let client_key_path = config
        .get_param::<String>("BHARATCODE_CLIENT_KEY_PATH")
        .ok();
    ...
    match (client_cert_path, client_key_path) {
        ...
        (Some(_), None) => {
            bail!(
                "Client certificate provided (BHARATCODE_CLIENT_CERT_PATH) but no private key (BHARATCODE_CLIENT_KEY_PATH)"
            );
        }
        (None, Some(_)) => {
            bail!(
                "Client private key provided (BHARATCODE_CLIENT_KEY_PATH) but no certificate (BHARATCODE_CLIENT_CERT_PATH)"
            );
        }
        (None, None) => {}
    }

    if let Ok(ca_cert_path) = config.get_param::<String>("BHARATCODE_CA_CERT_PATH") {
