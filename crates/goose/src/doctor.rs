...
async fn ensure_working_provider(
    ...
    let provider_name = config.get_bharatcode_provider().ok();
    let model_name = config.get_bharatcode_model().ok();
    ...
    Ok(Some(Message::assistant().with_text(format!(
        "**BharatCode Doctor**\n\n{}\n\n\
         No working provider found. Run `bharatcode configure` to set one up.",
        preamble,
    ))))
}
...
fn describe_error(e: &ProviderError) -> String {
    match e {
        ProviderError::Authentication(_) => {
            "Authentication failed — check your API key. Run `bharatcode configure` to update it."
                .to_string()
        }
