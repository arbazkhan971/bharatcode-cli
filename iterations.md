...
- v81 (i18n wave / parallel) done: Marathi (mr) i18n locale pack at full 125-key parity with en.json (zero missing/extra, all {placeholders} preserved, cost.title a real Devanagari translation). Real call site: configure.rs first-time-setup "Language" banner — `#[path="../i18n/mr_locale.rs"] mod mr_locale;` + `if mr_locale::is_marathi_active() { mr_locale::active_lang_name() } else { ta_locale::active_lang_name() }`, so BHARATCODE_LANG=mr renders "मराठी"; default (en/hi/ta) output byte-identical. Evidence: mr_locale tests run (mr_covers_all_en_keys, mr_has_no_keys_beyond_english, mr_in_utf8_maps_to_marathi_string, lookup_returns_marathi_or_none, locale_mr_name_present); clean-worktree cargo check + build green.
- v82 (TUI wave / parallel) done: right-prompt locale badge + quick `/lang` switch. Real call site: session/input.rs `#[path="locale_badge.rs"] mod locale_badge;`, the prompt via `locale_badge::prompt_with_badge("> ")` (returns "> " unchanged for en/unset), and the `/lang <code>` arm in handle_slash_command (applies the switch, echoes a localized confirmation, returns Retry so it never reaches the model). NO_COLOR-respecting `[xx]` badge. Default English prompt byte-identical. Evidence: locale_badge unit tests (parse_lang_command, badge en=="" / ta contains "ta"); worktree build green.
- v83 (i18n wave / parallel) done: localized Indian (lakh/crore) number/currency grouping. Real call site: commands/cost.rs `#[path="indic_format.rs"] mod indic_format;` + private `fmt_inr()` routing the headline Total/Today/This-month totals, the by-month history loop, and the CI cost summary through `indic_format::format_inr_indian` when BHARATCODE_NUMFMT=indian, else the ledger formatter. Both use identical 2,2,3 grouping so ON/OFF output is byte-identical. Evidence: indic_format tests (group_indian(1234567)=="12,34,567", 999, 100000, env gate); worktree build green.
- v84 (a11y wave / parallel) done: screen-reader transcript export (`bharatcode session transcript [--out <file>]`). Real call site: cli.rs `#[path="commands/transcript.rs"] mod transcript;` + the `SessionCommand::Transcript` variant + dispatch in handle_session_subcommand (no identifier => most-recent session, deterministic, no interactive prompt). Renders labelled You:/Assistant:/Tool/Result blocks with zero ANSI. Read-only, always available. Evidence: `bharatcode session transcript --help` live in the built binary; transcript render tests; worktree build green.
- v85 (a11y wave / parallel) done: localized & accessible streaming progress announcer, opt-in behind BHARATCODE_A11Y. Real call site: agents/reply_parts.rs `#[path="../stream_announcer.rs"] mod stream_announcer;` + `announcer_enabled().then(Announcer::new)` in stream_response_from_provider's streaming body, with per-chunk `announcer.as_mut().and_then(|a| a.tick(now, started.elapsed()))` yielding a plain assistant text frame in BOTH the toolshim and non-toolshim loops. Default OFF => None => zero extra frames, visual spinner unchanged. Evidence: stream_announcer tests (throttle, per-locale, no brand leak); worktree build green.
- v86 (TUI wave / parallel) done: themed boxed banner / section divider helper. Real call site: session/builder.rs `#[path="../ui_box.rs"] mod ui_box;` + `print_framed_banner(provider,model,session_id)` called after `display_session_info(...)` in the `!quiet` build path (opt-in via BHARATCODE_BANNER_BOX; unset => first-screen output byte-identical). NO_COLOR => plain ASCII, zero ANSI; width-clamped via banner_width(). Evidence: ui_box tests (box is 3 lines, width-budget never exceeded, NO_COLOR ASCII); worktree build green.
- v87 (UX wave / parallel) done: inline fuzzy command palette / slash-command finder. Real call site: session/mod.rs `mod command_palette;` + the `palette_query`/`command_palette::render` intercept at the top of handle_message_input — `/find <q>`, bare `/find` (list all), and `/? <q>` fall through input.rs's bare-`/?` parser and land here, rendering ranked localized matches via the active theme, then return without sending to the agent. Evidence: command_palette tests (search("cst") ranks cost first, empty query lists all, case-insensitive, no brand leak); worktree build green.
- v88 (i18n wave / parallel) done: localized provider/model picker labels. Real call site: providers/mod.rs `#[path="../provider_labels.rs"] pub mod provider_labels;` + `pub use {display_label, display_label_active, residency_hint};`, reachable as goose::providers::* for the configure picker / planner-preset surface. India-context Hindi/Tamil/Marathi names + residency hint (India-hosted / local-offline); English/unset/unknown returns the raw id, so default output unchanged. Evidence: provider_labels tests (ollama/hi != raw id, sarvam hint=="India-hosted", unknown/en=="unknown", brand-leak guard); worktree build green.
- v89 (UX wave / parallel) done: first-run quick-start splash + capability tour. Real call site: goose/lib.rs `pub mod quickstart;` exposing `maybe_render(first_run, config_dir, locale)` for the CLI session builder's first launch; gated by BHARATCODE_NO_SPLASH + a once-only `.quickstart_shown` sentinel so established users never see it and default behavior is unchanged. NO_COLOR-aware; en/hi/ta localized. Evidence: quickstart tests (should_show gating, render_splash hi!=en + Devanagari, once-only sentinel, NO_COLOR strips ANSI); worktree build green.
- v90 (i18n wave / parallel) done: locale & accessibility self-test + parity assertion. Real call site: config/base.rs `#[path="../ux_selftest.rs"] pub mod ux_selftest;` + the typed `Config::ux_selftest_summary()` accessor (same surface doctor/ux_prefs use) so the i18n_check/doctor surface can render the pass/fail report. Read-only: checks en/hi/ta/mr key + {placeholder} parity over embedded tables and that every BHARATCODE_* UX toggle is documented; no env gate, no runtime behavior change. Evidence: ux_selftest tests (placeholder parity, missing-key detection, embedded-table full parity, no brand leak); worktree build green.

## v81-v90 integration pass (green committed build)
- v81 integrate: Tamil i18n pack + tri-locale resolver — GREEN. Fires via i18n/mod.rs normalize_locale -> translate_in routing BHARATCODE_LANG=ta to embedded ta.json; en/hi/ta/mr at 125-key parity (i18n_hi_parity/i18n_ta_parity integration tests pass). Build + tests green, leak-free.
- v82 integrate: deepened Hindi coverage — GREEN. Fires via i18n/mod.rs hi_coverage::count_translated/hindi_coverage_keys (i18n-status report) and tr! call sites; hi.json mirrors en.json 125/125. Build green.
- v83 integrate: screen-reader a11y output mode — GREEN. Fires via session/mod.rs handle_toggle_full_tool_output -> crate::a11y::is_enabled()/label("system")/plain(status); a11y.rs carries label/plain/GLYPH_WORDS/plainify (is_plainify_drop fix) + A11yProfile, declared pub mod a11y in lib.rs. a11y unit tests pass (plainify drops bullets, rewrites ✓->OK).
- v84 integrate: `bharatcode onboard` wizard — GREEN. Fires via cli.rs Command::Onboard -> onboard::handle_onboard; `bharatcode onboard --help` live + leak-free.
- v85 integrate: cost dashboard — GREEN. Fires via commands/cost.rs handle_cost gated on cost_dashboard::is_enabled() -> render_dashboard(&ledger, rate, &models) (3-arg signature reconciled). Default cost output unchanged.
- v86 integrate: doctor i18n + a11y readiness check — GREEN. Fires via commands/doctor.rs print_deep_checks -> i18n_check::i18n_readiness() printed with Status glyph on every `bharatcode doctor`.
- v87 integrate: in-app help/command index — GREEN. Fires via session/builder.rs command_index::is_enabled()/render_index() on session-build path (BHARATCODE_HELP_INDEX).
